...
- **A delegating brief carries the principles, not just the task.** An agent decides by the standard it was given: hand it findings and a verification standard alone and it returns work that is rigorously checked and architecturally wrong. Every brief states the Code Style rules above (good code, not similar), the zero-compatibility premise, architecturally-correct over smallest-patch, and that production API existing for tests is a defect rather than a justification. Never write "preserve what is still true", "keep what still has callers", or any instruction that makes survival the default — they invert the burden of proof below.
