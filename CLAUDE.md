...
- A PR merges when the authoring session holds high confidence; the merge note names the merging model ("Merged-By: …"). Waiting for a lane to finish is never a merge condition: the lanes (docs/TESTING.md) run on the PR and again on main, and a failure they surface is fixed forward or reverted. Run only the suites a change affects, never the full unit lane by default. An independent fresh-context review is required only where the lanes are blind: release, signing, and attestation workflows or scripts; cryptographic or code-generation changes (the OpenSSL and Rust carries, custody and keychain code); entitlements. It runs in the background and must not block on agent messaging. CLAUDE.md is merged only by the maintainer.
