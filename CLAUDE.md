...
- **[Temporary — effective now, reconsider when the cleanup campaign closes] A delegating brief carries the principles, not just the task.** An agent decides by the standard it was given: hand it findings and a verification standard alone and it returns work that is rigorously checked and architecturally wrong. Every brief states the Code Style rules above (good code, not similar), the zero-compatibility premise, architecturally-correct over smallest-patch, and that production API existing for tests is a defect rather than a justification. Never write "preserve what is still true", "keep what still has callers", or any instruction that makes survival the default — they invert the burden of proof below.
- **[Temporary — effective now, reconsider when the cleanup campaign closes] Deletion is the default disposition; the burden of proof is on the keep.** A thing survives only when there is a stated positive reason it must exist — that something references it is not a reason, it is the next question (should the caller change?). Do not gate a deletion behind proving it dead first: a wrong deletion fails the lanes and is a revert, a wrong keep is permanent. Delete, run the lanes, learn. The exception is not a hesitation but a note about evidence — Release-only behaviour, signing and entitlement effects, and security checks no test asserts are invisible to the lanes, so those want better tests rather than caution.
