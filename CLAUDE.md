...
- A PR merges once the validation lanes (docs/TESTING.md) have passed and the authoring session holds high confidence; the merge note names the merging model ("Merged-By: …"). An independent fresh-context review is required only where the lanes are blind: release, signing, and attestation workflows or scripts; cryptographic or code-generation changes (the OpenSSL and Rust carries, custody and keychain code); entitlements. It runs in the background alongside the lanes and must not block on agent messaging. CLAUDE.md is merged only by the maintainer.
