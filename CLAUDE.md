...
- **[Temporary — in force until the maintainer revisits it, a review due after the cleanup campaign] A delegating brief carries the principles, not just the task.** An agent decides by the standard it was given: give it findings and a verification standard alone, and it returns work that is rigorously checked and architecturally wrong. Every brief states the Code Style rules above (good code, not similar), the zero-compatibility premise, the architecturally-correct-over-smallest-patch rule, and that a production API existing for tests is a defect rather than a justification. Never write "preserve what is still true", "keep what still has callers", or any other instruction that makes survival the default — each of them inverts the burden of proof below.
