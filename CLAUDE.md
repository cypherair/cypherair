...
- **Structure:** one type per file, grouped by feature; test doubles under `Tests/Support/SecurityMocks/` with `Mock*` names (Sources ships no mocks); all user strings in the String Catalog (remove `stale` keys, don't just unmark them).
