...
- **Key families:** Key generation offers four families — Portable Compatible (Profile A software key, GnuPG-compatible), Portable Modern (Profile B software key, RFC 9580), and Device-Bound Compatible/Modern (Secure Enclave custody, P-256 v4/v6, non-exportable). Profile A/B remains the technical vocabulary for the software configurations. Device-bound families are product-selectable (issue #501 Phase 7D); the release gate is satisfied (Phase 9) and they ship with the next stable release. See docs/PRD.md Section 3 and docs/SECURE_ENCLAVE_CUSTODY.md.
