...
- Phase 4 framework-hardening coverage in `ProtectedDataFrameworkTests` proves recovery dispatch is keyed by `ProtectedDataDomainID`, that mismatched handlers do not run, that `protected-framework-sentinel` second-domain create/delete/recovery paths are covered, and that shared-resource cleanup is derived from post-removal membership; this coverage is an AppData prerequisite, not part of the Contacts-internal PR numbering
- Phase 4 post-unlock orchestration coverage in `ProtectedDataFrameworkTests` proves that app unlock can open `protected-settings` and `protected-framework-sentinel` with the authenticated `LAContext`, while missing context, empty registry, no eligible registered domain, or pending mutation states do not read the root secret; this coverage remains owned by AppData Phase 4 even when later Contacts work consumes it
...
- that private-key control state belongs to the dedicated `private-key-control` protected domain, not an ordinary protected-settings payload
