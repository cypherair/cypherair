...
> Status: Archived historical review snapshot. This document records a point-in-time tutorial implementation audit and is no longer maintained as active current-state guidance.
...
> Original purpose: Verify the current tutorial implementation against the active rebuild specification and capture snapshot-scoped findings.
> Original date: 2026-04-10.
> Archive refresh: 2026-04-25.

Source spec: `TUTORIAL_REBUILD_SPEC.md`
...
- Current repository test inventory under `Tests/` and `UITests/`

Limitations:
- No new `xcodebuild test` run was executed for the original 2026-04-10 audit
- Archive refresh validation on 2026-04-25 ran `xcodebuild test -scheme CypherAir -testPlan CypherAir-UnitTests -destination 'platform=macOS' -only-testing:CypherAirTests/TutorialSessionStoreTests`; 41 tests passed
...
- As of archival, the tutorial product is stable enough that its long-term contracts now live in the active PRD, architecture, security, and testing documents instead of in a standalone active rebuild spec.
...
- Tutorial UI regression coverage is partial. Current coverage is strong at the session-store and configuration level, and macOS smoke coverage exercises selected tutorial launch/follow-up routes, but first-run handoff, modal continuity, compact-width navigation, and completion behavior still lack broad end-to-end UI coverage.
...
| Tutorial UI regression coverage | Partial coverage | Current tutorial coverage lives primarily in `Tests/ServiceTests/TutorialSessionStoreTests.swift`, and `UITests/MacUISmokeTests.swift` now includes selected tutorial launch, sandbox key-generation, key-detail, QR, backup, and disabled-route smoke paths. | The main tutorial lifecycle is reasonably unit-tested, and macOS smoke coverage has improved, but first-run handoff, modal continuity, compact-width navigation, and completion behavior are not broadly protected by end-to-end UI tests. |
...
3. Expand tutorial UI coverage beyond the current macOS smoke paths to cover first-run handoff, skip path, sequential completion, return-to-hub, and completion exit.
