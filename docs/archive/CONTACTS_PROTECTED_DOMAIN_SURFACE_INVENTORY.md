...
> **Status:** Archived historical Contacts surface checklist.
> **Archived on:** 2026-05-10.
> **Archival reason:** Durable current-state Contacts access, mutation, and regression requirements have been consolidated into long-term docs.
> **Successor documents:** [ARCHITECTURE](../ARCHITECTURE.md) · [TDD](../TDD.md) · [TESTING](../TESTING.md) · [CODE_REVIEW](../CODE_REVIEW.md) · [PERSISTED_STATE_INVENTORY](../PERSISTED_STATE_INVENTORY.md)
...
| Tag management | `ContactsView` tag filters/search suggestions + `ContactDetailView` tag add/remove + `ContactService.addTag/removeTag` | mutation | yes | yes | Protected-domain unlock required; legacy compatibility is display/search only | Contacts PR8 | Tags belong to `ContactIdentity`; write path normalizes trim/collapsed whitespace, suppresses case-insensitive duplicates, and prunes unused tag records |
| Recipient-list management | `RecipientListsView` / `RecipientListDetailView` + Encrypt list picker + `ContactService` recipient-list CRUD | mutation | yes | yes | Protected-domain unlock required; Encrypt disables empty/invalid lists | Contacts PR8 | Lists bind to `ContactIdentity`, not fingerprints; Encrypt resolves current member contact IDs and fails closed if any member lacks a preferred encryptable key |
...
Contacts PR7 package exchange is withdrawn and has no active inventory rows. Contacts PR8 covers in-memory search, tag management/filtering/suggestions, recipient-list management, and Encrypt recipient-list selection over the protected `contacts` domain. Any future complete Contacts backup or device migration must be designed separately as a mandatory encrypted feature before it receives inventory coverage.
