...
- **Review depth scales with blast radius.** Small mechanical changes need nothing beyond the validation lanes. A normal PR gets one independent fresh-context verification (its stage-verify). Security-critical and governance changes additionally get the maintainer's own review before merge (§3). Campaign stages add the per-stage pass above. A whole-codebase review is a deliberate, rare event — multi-agent workflows with assess-first cost control and a mechanical floor (e.g. `periphery`) beneath model judgment.
...
- **PR and merge.** Git mechanics live in CLAUDE.md (regular merge commit, signed conventional commits; sessions manage branches, worktrees, and delegation themselves). A PR merges once its verification has passed and both the authoring agent and the merging session hold high confidence — agent merges leave a note naming the merging model (e.g. "Merged by Claude (Fable 5)"). Security-critical changes (§3) and the governance documents themselves (CLAUDE.md, AGENTS.md, this doc) are always reviewed and merged by the maintainer.
