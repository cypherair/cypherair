...
Authentication presentation is the **system authentication sheet** for both subsystems on every platform. (A macOS 27 developer-beta regression denied embedded LocalAuthentication UI to non-Apple-signed processes — LA -1007, introduced by build `26A5353q` — and was lifted by build `26A5388g`, verified 2026-07-23 by re-running the archived PR #496 probe suite: 8/8 clean, including the `evaluateAccessControl` custody route. macOS in-window authentication adoption is tracked in issue #724.) App lock is an explicit state machine (`AppLockController`: `.locked`/`.authenticating`/`.unlocked`) with per-platform away events. **Each system prompt runs inside a short operation-prompt session** covering the prompt plus the immediately following Keychain/Secure Enclave call that consumes the same `LAContext`; longer work (PGP generation, import parsing, journaling, commits, reset I/O, UI updates) stays outside the session, so prompt-lifecycle resigns are deferred while genuine away events under grace=0 still relock immediately. Key-expiry modification authenticates once per action with an `LAContext` confined to that action.
