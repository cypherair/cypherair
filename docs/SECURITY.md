...
- **A Secure Enclave route never falls back to software secret-certificate material** — every non-matching path returns a blocked resolution. The decrypt Phase 1/Phase 2 boundary is preserved: Phase 1 reads session-key packets only, is unauthenticated, and the matched-key guard runs before any private-key access. Opening a password message stays inside Phase 1's guarantee for the whole operation — it uses no private key and raises no authentication prompt (§7).
...
Argon2id can also run when **opening a password-encrypted (SKESK) message**, where the parameters are the sender's. Writing one never does: the app emits Sequoia's default Iterated+Salted S2K for a password message, so an outgoing password message costs the recipient nothing to open regardless of what tool they use.
...
**Memory-safety guard, all three paths.** `Argon2idMemoryGuard` owns one definition of what this process can afford: 75% of the headroom under the memory limit it has actually been granted, read per operation and never cached. Key export and import ask it to *validate* a requirement the engine states — parsed from the incoming key on import, declared from the suite on export — and refuse above the budget, preventing Jetsam termination mid-derivation. Answering the export question from the suite alone is what lets that check run before the authentication prompt and before the private key leaves the enclave.

The message path cannot work that way, because the parameters are the sender's and a message may carry several password slots at different costs. So the budget travels *into* the engine (`decrypt_with_password`'s `affordable_memory_kib`) and is applied per slot, before that slot's KDF runs: a slot beyond it is refused rather than derived, and a message carrying one unaffordable slot alongside an affordable one still opens through the latter. The engine's own ceilings stay and are never widened by the host — 2 GiB of Argon2 memory, 16 passes, 16 SKESK packets — so the effective bound is the lower of the format's and the device's. The 2 GiB figure admits RFC 9106's primary recommendation exactly, which is what our own key export emits, and rejects the OOM range above it.

**The passphrase is the other half of the cost.** Argon2id sets what one guess costs; it does nothing about a secret that is guessed early. Every screen where the user **chooses** a passphrase or password — private-key backup, and the password on an outgoing password message — applies the same two requirements through the same component: a minimum length, and no character repeated past a short run, with a generated ~116-bit value offered as the primary path. The requirements are deliberately not a strength score: a score needs frequency corpora this app will not ship or download, and generation is what actually answers guessability. Entering a secret that already protects an artifact — a backup being restored, a password message being read — is never gated: that protection was fixed when the artifact was written, and refusing to open it would only lock the user out.

**A password message involves no key of the sender's.** It is not signed, and no self copy is encrypted, so writing one raises no authentication prompt and unwraps nothing; reading one likewise touches no private key. That is what makes the Encrypt screen's exclusive choice ([PRODUCT.md](PRODUCT.md) §5) a security statement and not only a UI one: there is no state in which a message carries both a password slot and a recipient's session key.
