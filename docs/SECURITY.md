...
Authentication presentation is the **system authentication sheet** for both subsystems on every platform. (macOS 27 betas deny embedded LocalAuthentication UI to non-Apple-signed processes — LA -1007, since build `26A5353q` — and the denial is **environment-dependent**: on build `26A5388g` the archived PR #496 probe suite passes 8/8 inside the Xcode unit-test host, but the installed app build is still denied — reproduced 2026-07-23 by the maintainer, console-log-confirmed, and the reason PR #728's in-window unlock was reverted after merge. Do not trust test-host probe results for this gate; verify with the real app build. macOS in-window authentication adoption is parked, tracked in issue #724.) App lock is an explicit state machine (`AppLockController`: `.locked`/`.authenticating`/`.unlocked`) with per-platform away events. **Each system prompt runs inside a short operation-prompt session** covering the prompt plus the immediately following Keychain/Secure Enclave call that consumes the same `LAContext`; longer work (PGP generation, import parsing, journaling, commits, reset I/O, UI updates) stays outside the session, so prompt-lifecycle resigns are deferred while genuine away events under grace=0 still relock immediately. Key-expiry modification authenticates once per action with an `LAContext` confined to that action.
