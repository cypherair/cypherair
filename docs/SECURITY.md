...
3. **Out of scope, stated plainly:** an unlocked session against a kernel-level memory reader, and the passphrase against an implant capturing keystrokes at the moment it is typed. Relock bounds the first; nothing a third-party app can do bounds the second. Device-bound private material stays unextractable even then. Deletion is detected and ends in a reset; rollback of a file is not detected and yields older state, never forged state.
...
- **Passphrase change is one atomic row update.** Run the unlock chain, take the new passphrase, draw a new salt, create a new wrapping key under the new stretched value, seal the same root secret, and replace the sealed root row's value in place; the Keychain applies the update atomically, so the old passphrase stays in force until the new value exists. The old wrapping key lived only inside the old envelope and is gone with it. A cancelled prompt leaves no intent behind.
...
- **Domains open only after unlock.** Pre-unlock startup may check which files exist but must not derive the wrapping root key, derive any domain key, or open protected payloads.
- **Domain keys are derived, never stored.** Each domain's key is HKDF over the wrapping root key with the domain label; derived keys and decrypted payloads are session-local.
- **One sealed file per domain, replaced atomically.** A save writes the new snapshot to a temporary file and renames it over the old one; a leftover temporary file is ignored and swept. There are no generations, no registry, and no rollback watermark: the set of domains is fixed by the app, and the sealed root's existence is the marker that onboarding completed.
- **One integrity-failure state, never a silent default.** If the sealed root exists and any domain file is missing or fails to authenticate, or domain files exist without a sealed root, the app opens into a single screen that names what is intact and what is damaged, allows export of portable keys whose list and envelopes are intact, and offers Reset All Local Data. Nothing is repaired or re-bootstrapped per domain; no setting falls back to a default, and encryption never silently uses a default encrypt-to-self value. Reinstalling on iOS lands here too, because enclave rows outlive the container, and the user resets once.
- **Rollback is out of scope.** A restored file shows older state, never forged state, because every file still authenticates; a rolled-back key list that names a deleted key fails closed at use, because the list is never rebuilt from envelope rows.
...
- **File protection is verified, not assumed** — every domain file and scratch write; storage outside the app-owned container is never a fallback.
