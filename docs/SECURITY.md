...
Authentication presentation is the **system authentication sheet** for both subsystems on every platform, with one exception: the **macOS app-session unlock** authenticates **in-window** (issue #724 stage 2) — the shield lock surface hosts an embedded biometric evaluation (`LAAuthenticationView` bound to the evaluated `LAContext`, so unlocking does not resign the app), and the Standard-mode "Use Password…" action runs a detached system-sheet evaluation (High Security remains biometric-only). macOS per-operation private-key prompts and every other platform stay on the system sheet; the per-operation stage remains tracked in issue #724. (A macOS 27 developer-beta regression denied embedded LocalAuthentication UI to non-Apple-signed processes — LA -1007, introduced by build `26A5353q` — and was lifted by build `26A5388g`, verified 2026-07-23 by re-running the archived PR #496 probe suite: 8/8 clean, including the `evaluateAccessControl` custody route.) App lock is an explicit state machine (`AppLockController`: `.locked`/`.authenticating`/`.unlocked`) with per-platform away events. **Each system prompt runs inside a short operation-prompt session** covering the prompt plus the immediately following Keychain/Secure Enclave call that consumes the same `LAContext`; longer work (PGP generation, import parsing, journaling, commits, reset I/O, UI updates) stays outside the session, so prompt-lifecycle resigns are deferred while genuine away events under grace=0 still relock immediately. Key-expiry modification authenticates once per action with an `LAContext` confined to that action.
...
- Tutorial private-key protection runs on the real Secure Enclave through `EphemeralKeyWrappingCustody`, behind a real `AuthenticationManager`: promptless nil-access-control wrapping keys whose representations exist only inside envelope rows, sealed and opened by the production `PrivateKeyEnvelope` path. There is no software fallback — without a Secure Enclave, sandbox custody fails closed. The envelope rows live in the in-memory `EphemeralKeychainStore` (real duplicate/not-found semantics; wiped with zeroization at cleanup), the contacts wrapping root key is drawn from `SecRandomCopyBytes` per session and zeroized at cleanup, and the device-bound custody seams are wired to inert fail-closed conformances so the sandbox cannot reach real custody state by construction.
- Containment: the ephemeral stores throw their own error types and never impersonate the production `KeychainError`. The same rule binds the `Mock*` test doubles, which live only in the test target (`Tests/Support/SecurityMocks/`) and are never compiled into the app.
