...
- **File protection is a creation attribute of every directory the app owns.** The ProtectedData root, each domain directory, each temporary-operation directory, and each export-staging directory is created with the complete-protection class as a creation attribute, covering the intermediate directories made along the way; everything born inside inherits the class — including the files the app does not create itself, such as the SQLCipher database with its journal and the encryption engine's streaming outputs and success-only `.tmp` files. Nothing re-applies protection to a file after the fact; the one runtime check is at root creation, asserting the class the root came out with is the class intended. On iOS, iPadOS and visionOS the container's own default is raised to the same class by the `com.apple.developer.default-data-protection` entitlement in `CypherAir.entitlements`, so a file created wholly outside the app's reach — a preferences file the system replaces — is born protected too; the entitlement is invisible to the macOS lanes, so the App Store candidate validation script asserts it ships. macOS keeps the directory class — the boundary there is signing out rather than the lock screen. Storage outside the app-owned container is never a fallback.
- **An operation in flight stops when protected data is about to become unavailable** — the device locked past backgrounding. The stop rides the operation's own cancellation path, which erases the partial output, and the person is told the operation was interrupted and can be run again. While an operation runs the app holds off idle sleep, so an ordinary long encrypt never meets the lock; no file is held open across a lock, and no directory takes a weaker class for the duration.
