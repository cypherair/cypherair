...
- **Revocation.** Revocation signatures are stored as binary packets and armored on demand; export uses only the stored artifact and **fails closed when it is missing — a missing artifact is never regenerated**. The import path generates a key-level revocation for the imported key. Subkey and User ID revocations are generated on demand and create no persisted selective-revocation history. Certification persistence never inserts signatures into a stored contact certificate and never changes manual verification state. It also confers no trust of its own: a stored certification records a signature, and whether that signature counts for anything is derived separately, live, from the user's current verifications ([PRODUCT.md](PRODUCT.md) §5) — one hop, over imported certificates only, never a discovered or transitive web of trust.
...
- **Contacts:** manual verification is a local fingerprint assertion, not OpenPGP certification, and it is the only anchor any trust the app states hangs from; certification-signature export is an explicit artifact boundary, not a Contacts backup. The mandatory-encrypted rule for any future Contacts exchange is product law ([PRODUCT.md](PRODUCT.md) §8).
