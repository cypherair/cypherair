...
Argon2id S2K applies to **private-key export and passphrase-protected import only** (the v6 portable families) — never to routine decrypt/sign, and never to Portable Legacy (Iterated+Salted, mode 3). This scope prevents the recurring "why is decrypt slow" misdiagnosis. The shipped parameter set is RFC 9106's primary recommendation — `t=1, p=4, m=2^21 KiB (2 GiB)` — in RFC 9580 encodings. `pgp-mobile/src/keys/s2k.rs` owns those constants, builds the specifier the packets carry, and publishes the same figure to the app through `export_s2k_params`, so the cost the guard checks is the cost the KDF runs under.

**Memory-safety guard, both directions.** Export derives under the same parameters as import and can be terminated mid-derivation just as easily, so both are checked. The KiB requirement comes from the engine — parsed from the incoming key on import, declared from the suite on export — and Swift refuses above 75% of available memory (`os_proc_available_memory()` on iOS/iPadOS/visionOS; total physical memory on macOS, which has no Jetsam), preventing Jetsam termination mid-derivation (`Sources/Security/Argon2idMemoryGuard.swift`; refusal copy in the String Catalog). Answering the export question from the suite alone is what lets the check run before the authentication prompt and before the private key leaves the enclave.
