...
Phase 3 by itself did not mean all settings had moved. Phase 7 PR 2 owns the ordinary-settings expansion; other protected-after-unlock surfaces remain Phase 7 work unless another phase explicitly owns them.
...
- `TutorialSandboxContainer` creates `tmp/CypherAirGuidedTutorial-<UUID>/` with verified `.complete` protection and still removes the current tutorial directory plus current tutorial defaults suite on reset/finish.
- Startup cleanup and Reset All Local Data enumerate the app Preferences directory for `com.cypherair.tutorial.*.plist`, remove matching persistent domains, and delete residual plists so tutorial defaults suites orphaned by crash or system kill do not persist.
