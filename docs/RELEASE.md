...
4. **WF1 — `PgpMobile XCFramework`** (start condition: tag `cypherair-v*-build*`): audits Rust dependencies (`cargo audit --deny warnings`), downloads and provenance-verifies the pinned stage1 toolchain (digest pin, release immutability, asset attestations), builds the arm64e `PgpMobile.xcframework` from source with it, restores the pinned SQLCipher dependency with mandatory attestation verification, packages the six SDK/compliance assets, records the SQLCipher pin in the compliance manifest, creates the stable GitHub Release as a **draft**, and starts WF2 via the App Store Connect API.
5. **WF2 — `CypherAir Release`** (start condition: manual/API only — started by WF1): downloads and `shasum`-verifies the exact XCFramework from the draft, re-restores SQLCipher with attestation, runs the App Store candidate validator with authenticated `gh` (draft releases are invisible to unauthenticated lookups) and writes a commit-bound verdict file, then archives iOS / macOS / visionOS with cloud-managed signing — the archive scheme pre-action re-runs the validator after the post-clone hook has destroyed its scoped gh credentials, so it consumes the verdict instead of re-querying GitHub — delivers to TestFlight (internal), attaches the App Store upload artifacts, and **publishes** the draft once all platform artifacts are present.
