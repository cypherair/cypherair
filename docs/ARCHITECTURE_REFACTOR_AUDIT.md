...
| UI / ScreenModel adoption is uneven | Newer flows use ScreenModels, for example `DecryptView` constructs `DecryptScreenModel` in [Sources/App/Decrypt/DecryptView.swift](../Sources/App/Decrypt/DecryptView.swift), Phase 3 PR3B moved Contact Detail mutations into [Sources/App/Contacts/ContactDetailScreenModel.swift](../Sources/App/Contacts/ContactDetailScreenModel.swift), and Phase 4 PR4A moved key generation, import, backup, expiry modification, key detail, and selective revocation behind key-route ScreenModels. Remaining examples include QR display/generation in [Sources/App/Contacts/QRDisplayView.swift](../Sources/App/Contacts/QRDisplayView.swift) and settings-adjacent security/reset flows. | The PR4A key-management route scope is resolved for normal production views. Some Contacts/QR and Settings-adjacent routes still keep async workflow, service calls, mutation handling, or security-facing orchestration for later Phase 4/5 work. | Medium. Behavior remains localized, but remaining view-level orchestration makes future workflow changes harder to test consistently. |
