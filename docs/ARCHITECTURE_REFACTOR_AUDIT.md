...
> review coverage for key-route view orchestration.
...
> scope, and added review coverage for Contacts route view orchestration.
> PR4C update: 2026-05-19 promoted decryption Phase 1 results to app-owned
> models, replaced file-operation ScreenModel action seams with app-owned
> request/result wrappers, and reviewed service phase, progress, and
> temporary-artifact internals in ScreenModel public APIs.
...
- Phase 2 PR2C validation found SwiftUI color/icon/status presentation and
  ...
  presentation helpers, with review and behavior coverage carrying regression
  prevention.
...
- Phase 4 PR4C promoted decryption Phase 1 result values into app-owned
...
- Phase 4 close-out accepted PR4A through PR4C as complete for the owned scope:
  ...
  output/configuration checks have focused test coverage and recorded review
  expectations. Remaining Settings/Security/ProtectedData/App-composition
  findings stay recorded below as Phase 5 debt rather than Phase 4 blockers.
...
| UI / ScreenModel adoption is uneven | Newer flows use ScreenModels, for example `DecryptView` constructs `DecryptScreenModel` in [Sources/App/Decrypt/DecryptView.swift](../Sources/App/Decrypt/DecryptView.swift), Phase 3 PR3B moved Contact Detail mutations into [Sources/App/Contacts/ContactDetailScreenModel.swift](../Sources/App/Contacts/ContactDetailScreenModel.swift), Phase 4 PR4A moved key generation, import, backup, expiry modification, key detail, and selective revocation behind key-route ScreenModels, and Phase 4 PR4B moved QR display/generation behind [Sources/App/Contacts/QRDisplayScreenModel.swift](../Sources/App/Contacts/QRDisplayScreenModel.swift) while tightening Add Contact QR-photo processing in [Sources/App/Contacts/AddContactScreenModel.swift](../Sources/App/Contacts/AddContactScreenModel.swift). | The PR4A key-management route scope and PR4B Contacts/QR route scope are resolved for normal production views. Settings-adjacent routes still keep security-facing orchestration for later Phase 5 work, with the Settings-facing ScreenModel cleanup planned after PR5A-PR5C establish narrower workflow boundaries. | Medium. Behavior remains localized, but remaining settings-adjacent orchestration makes future workflow changes harder to test consistently. |
| Phase 4 ScreenModel public API cleanup is complete for service phase and file-operation seams | `DecryptScreenModel` now uses app-owned `DecryptionPhase1Result` and `FileDecryptionPhase1Result` values from [Sources/Models/DecryptionPhase1Result.swift](../Sources/Models/DecryptionPhase1Result.swift) and [Sources/Models/FileDecryptionPhase1Result.swift](../Sources/Models/FileDecryptionPhase1Result.swift). Encrypt, Decrypt, Sign, and Verify ScreenModel file-operation action signatures use app-owned request/result wrappers and no longer expose `FileProgressReporter` or `AppTemporaryArtifact`. Contact certificate ScreenModels continue to expose app-owned `OpenPGPCertificationKind` and `CertificateSignatureVerification` state. | The former Phase 4 PR4C service-phase, progress, and temporary-artifact action-seam leak is resolved for current production ScreenModels. `OperationController`, `FileProgressReporter`, and temporary artifact storage remain App/Service workflow infrastructure rather than ScreenModel public action contracts. | Low to Medium residual risk, managed through ScreenModel API review and file-operation behavior tests. |
...
The 2026-05-18 PR3A refresh checked that production `[Contact]` references
were limited to `ContactService`, `ContactsCompatibilityMapper`,
...
production-source review and targeted Contacts tests for empty
...
review expectation keeping key-management workflow calls out of key route Views.
...
Contact QR-photo loader tests, and documented the review expectation keeping
contact/QR workflow calls out of Contacts route Views.
The 2026-05-19 PR4C refresh checked that decryption Phase 1 result contracts are
app-owned, that Encrypt/Decrypt/Sign/Verify ScreenModel file-operation action
signatures no longer expose `FileProgressReporter` or `AppTemporaryArtifact`,
and that review plus focused tests cover regressions for those ScreenModel
public API seams.
