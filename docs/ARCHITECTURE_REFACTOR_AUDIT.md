...
> PR2B validation; other rows remain the original audit snapshot unless
> explicitly noted.
...
- Generated result and enum types such as `KeyInfo`, `SignatureStatus`,
  `SignatureVerificationState`, `UserIdSelectorInput`, `CertificationKind`,
  and `KeyProfile` appear above a dedicated adapter boundary in Models,
  Views, ScreenModels, and UI-facing helpers.
...
The 2026-05-15 PR2B refresh also checked that production `Sources/App` and
`Sources/Models` no longer reference `PgpError`, and that `PGPErrorMapper`
