...
- The remaining flat `[Contact]` runtime state is compatibility/projection debt
  inside `ContactService`, `ContactsCompatibilityMapper`,
  `ContactsLegacyMigrationSource`, and `ContactImportMatcher` until Phase 3C/3D.
...
| Legacy `Contact` projection remains active runtime state | `ContactService` still stores a compatibility `contacts: [Contact]` projection while also maintaining `runtimeSnapshot: ContactsDomainSnapshot?`; protected-domain open projects the snapshot back through `ContactsCompatibilityMapper`. `ContactImportMatcher` and `ContactsLegacyMigrationSource` still need flat contacts for legacy conflict detection and migration input. | Phase 3 PR3A moved primary app/service/UI call sites to `ContactImportResult`, `ContactIdentitySummary`, `ContactKeySummary`, `ContactKeyRecord`, and `ContactsVerificationContext`, but the internal compatibility projection remains until Phase 3C/3D. | Medium to High. User-facing contracts are cleaner, but two runtime representations can still drift inside the Contacts facade and migration layer. |
| Contacts compatibility mapper is no longer the normal result contract but still backs projection | Protected imports now return summary/key result contracts, and verification/signature flows use `ContactKeyRecord` rather than flat contacts. The mapper itself still converts snapshots into `[Contact]`, and `ContactService` still exposes legacy-only helpers for compatibility and tests. | The former app-facing `AddContactResult`/flat-contact import contract is removed. Remaining debt is projection removal and retiring legacy `availableContacts`/`availableContact(...)` surfaces once Phase 3C/3D completes storage/runtime cleanup. | Medium. Compatibility is narrower, but the facade still carries legacy vocabulary internally. |
