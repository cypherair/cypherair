...
| Phase 4 ScreenModel public API cleanup is complete for service phase and file-operation seams | `DecryptScreenModel` now uses app-owned `DecryptionPhase1Result` and `FileDecryptionPhase1Result` values from [Sources/Models/DecryptionPhase1Result.swift](../Sources/Models/DecryptionPhase1Result.swift) and [Sources/Models/FileDecryptionPhase1Result.swift](../Sources/Models/FileDecryptionPhase1Result.swift). Encrypt, Decrypt, Sign, and Verify ScreenModel file-operation action signatures use app-owned request/result wrappers and no longer expose `FileProgressReporter` or `AppTemporaryArtifact`. Contact certificate ScreenModels continue to expose app-owned `OpenPGPCertificationKind` and `CertificateSignatureVerification` state. | The former Phase 4 PR4C service-phase, progress, and temporary-artifact action-seam leak is resolved for current production ScreenModels. `OperationController`, `FileProgressReporter`, and temporary artifact storage remain App/Service workflow infrastructure rather than ScreenModel public action contracts. | Low to Medium residual risk, guarded by source-audit coverage blocking service phase, progress, and temporary-artifact internals in ScreenModel sources. |
