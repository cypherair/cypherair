...
Current stable app-build release ordering and the shared source/compliance asset contract are documented in [RELEASE.md](RELEASE.md) and [RELEASE.md](RELEASE.md). This section records the current technical library selection, not a final legal conclusion about distribution compatibility.
...
- Documentation updates for storage or migration changes follow the update triggers in [WORKFLOW.md](WORKFLOW.md) Section 6, with [PERSISTED_STATE_INVENTORY.md](PERSISTED_STATE_INVENTORY.md) as the row-level inventory.
