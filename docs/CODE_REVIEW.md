...
- [ ] Secure Enclave custody generation stays hardware-guarded at the composition root, enrolled in one operation-prompt session, returns public-only OpenPGP material, verifies external P-256 signer responses against the signing public key and digest, and never persists Apple handle locators, access-control policy, response-file state, or secret certificates
