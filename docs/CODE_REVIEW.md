...
- [ ] Secure Enclave custody handle changes keep signing and key-agreement handles distinct, use `kSecClassKey`/`kSecAttrTokenIDSecureEnclave`, preserve biometrics-only private-key usage access control, and never reuse the retired multi-row software-custody wrapping scheme
...
- [ ] Secure Enclave custody hardware-evidence changes remain guarded in the device-only test plan and skip when Secure Enclave or biometrics are unavailable
- [ ] Secure Enclave custody generation stays hardware-guarded at the composition root, enrolls only custody authorization and authorized handle-load work in a short operation-prompt session, returns public-only OpenPGP material, verifies external P-256 signer responses against the signing public key and digest, and never persists Apple handle locators, access-control policy, response-file state, or secret certificates
- [ ] Secure Enclave custody recovery derives expected handles from public certificate bindings, classifies metadata/handle disagreement with sanitized categories only, and does not delete startup orphan handles outside Reset All Local Data or an explicit recovery action
...
- [ ] ProtectedData changes preserve the app-data/private-key-material boundary: no SE-wrapped private-key envelope bytes are copied into ProtectedData payloads
