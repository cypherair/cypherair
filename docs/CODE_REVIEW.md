...
- [ ] ProtectedData changes that migrate a persisted surface update `PERSISTED_STATE_INVENTORY.md`, `ARCHITECTURE.md`, `SECURITY.md`, `TDD.md`, `TESTING.md`, and `CODE_REVIEW.md` as needed; archived Contacts planning documents are historical context only and are not active current-state update targets
- [ ] Contacts changes preserve the protected `contacts` domain as the source of truth: no legacy/quarantine fallback, no plaintext derivative caches, no package/social-graph export, and no bypass of locked/recovery/framework-unavailable availability states
- [ ] Contacts changes preserve person/key semantics: merge keeps per-key manual verification and certification state, historical keys remain signer-recognition only, recipient lists resolve through preferred encryptable keys, and certification artifacts stay distinct from manual verification
