...
- **A weak passphrase is refused, not warned about.** Where the user *chooses* a passphrase — today, private-key backup — the app declines below its bar rather than letting a warning be dismissed, shows strength while typing, and offers a generated passphrase as the first option instead of asking for a better invention. Generation produces uniform characters rather than words, so no wordlist ships and no language is privileged ([SECURITY.md](SECURITY.md) §7).
