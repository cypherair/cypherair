...
- **An unplaceable certificate is refused, never relabeled.** The engine classifies a certificate from its algorithm, curve, and version together and is allowed to refuse; key and contact import surface an explicit unsupported-algorithm error for a certificate the engine cannot place (e.g. RSA), rather than admitting it under an approximated suite.
