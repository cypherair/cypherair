...
**Minimum device: 8 GB RAM.** Nothing in code enforces this — it is a support promise, and it becomes load-bearing under the decided Argon2id 2 GiB move (docs/SECURITY.md §7).
...
- **No backup flow and no backup badge for device-bound keys.** Their private material is not exportable in any form (docs/CUSTODY.md); the revocation certificate is the primary export action for a device-bound key.
...
- **Software (portable) keys survive through backups** — export is passphrase-protected, and losing the wrapped copy is recoverable only from such a backup (docs/SECURITY.md §3).
...
- **The privacy cover is not the lock.** When the app is not foreground-active it shows an opaque, app-identified cover — purely visual, with no authentication role; the lock surface is a separate state driven by the app-lock state machine (docs/SECURITY.md §4–§5).
...
- **A message not addressed to you never triggers an authentication prompt.** Decrypt Phase 1 matches recipients against public certificates only and fails without touching any private key (docs/SECURITY.md §3).
- **Signature verification during decryption is graded, not binary** — a bad or unknown signature is reported alongside the plaintext, not used to suppress it. Standalone verification hard-fails instead.
- **Every key ships with a revocation certificate**, generated at creation (and at import) and exportable from the key detail page.
- **Tutorial state never touches the real workspace**, and the tutorial is not a prerequisite for key generation. Isolation rules: docs/SECURITY.md §6.
...
- **Post-Quantum families (RFC 9980)** — interop target is the Sequoia lineage; they make **no GnuPG claim**, and product copy must never imply one. Full interop position and red lines: docs/CUSTODY.md §8.
...
- The **quantum-safety badge is on the result surface**, derived from the produced artifact's session-key packets — never from the live recipient selection (docs/SECURITY.md §2). Mixed recipient sets get a visible not-fully-quantum-safe state.
...
- Decrypted file previews live in the app temp directory and are cleaned **at next launch** — there is no termination-time sweep today. **Roadmap:** the owned temp subtree + launch-sweep design replaces this cleanup model.
