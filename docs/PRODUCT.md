...
- **Every copy is device-only and expires after five minutes.** One rule for everything the app copies — ciphertext, signed text, public keys, fingerprints, URLs: the write never reaches another device through Handoff or Universal Clipboard, and the copy leaves the clipboard five minutes later. iPhone, iPad and Vision Pro let the system remove it; on Mac the app clears its own copy, and only while the clipboard still holds it, never something copied elsewhere in the meantime ([SECURITY.md](SECURITY.md) §9). The Encrypt and Sign result copies state this promise in the shared clipboard safety notice (on by default, silenceable as a protected setting); the other copy buttons confirm in place per the rule below. A fingerprint copied from a context menu confirms nothing at all, because the menu — and with it the control that would confirm — is gone before there is anything to show.
- **Success is confirmed in place; a pop-up only ever asks a question or reports a failure.** A quick success — copying, today — is acknowledged by the control the user just tapped: it becomes the confirmation for a moment, then returns to rest. One shared control does this everywhere, so no screen invents its own: a checkmark that swaps in with a small symbol animation, and on iPhone a gentle success haptic. Reduce Motion drops the animation and keeps the haptic, which is not motion. Nothing the user succeeded at asks to be dismissed, and a finished encrypt, decrypt, sign or verify settles its result into place rather than snapping in.
