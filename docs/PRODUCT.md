...
- **Key expiry is the user's choice on both paths, including no expiry at all.** Generation offers a fixed set of whole-year terms plus Never; the modify sheet reaches the same ceiling but takes any date under it, down to the next day, or drops the expiry outright. No expiry means exactly that: the engine is told the key has none, not handed a distant date. What is offered, what is accepted, and the ceiling over both are stated once in code (`Sources/Models/Keys/KeyExpiryPolicy.swift`) and read by both surfaces — the two paths differ in how they ask and how finely, not in what they will set.
