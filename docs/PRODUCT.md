...
- **No password protection for files.** Password (SKESK) protection is offered for text messages only, in both directions (§5). The engine encrypts and decrypts a password message whole, in memory, and has no streaming form of it, so File mode stays recipient-key only rather than offering a choice that would fail on a large file.
...
- **Message format is never a manual choice.** The engine selects it from what the recipient certificates advertise: AEAD only when every certificate the message is encrypted to advertises SEIPDv2 (AGENTS.md Hard Constraint 8). For every key CypherAir generates that tracks the key version exactly; an imported certificate may advertise otherwise, and the advertised capability is what decides. A password message has no certificate to advertise anything, so it takes SEIPDv1 — the container every OpenPGP tool can open, which is the point of sending one.
- **A message is protected by recipient keys or by a password, never both.** The Encrypt screen makes it one exclusive choice: picking Password replaces the recipient chooser outright, so a combined key-and-password message is not expressible. A password message uses none of the sender's keys — it is not signed and no self copy is made, and the screen says so where the choice is made, alongside the plain statement that such a message is exactly as strong as its password. Chosen passwords face the same refusal and the same generator as the backup passphrase ([SECURITY.md](SECURITY.md) §7). Reading is symmetric and needs no separate surface: Decrypt inspects a message once and either names the key that opens it or asks for the password, and a password message never raises an authentication prompt because it touches no private key.
...
- **A message not addressed to you never triggers an authentication prompt.** Decrypt Phase 1 reads the message's session-key packets against public certificates only, and fails — or offers the password route — without touching any private key ([SECURITY.md](SECURITY.md) §3).
...
- **A weak passphrase is refused, not warned about.** Where the user *chooses* a passphrase or password — private-key backup, and the password on an outgoing password message — the app declines rather than letting a warning be dismissed, and offers a generated value as the first option instead of asking for a better invention. Entering one that already protects something (a backup being restored, a password message being read) is never gated. What it asks for is two plain requirements shown as they are met, never a strength score to argue with; generation produces uniform characters rather than words, so no wordlist ships and no language is privileged ([SECURITY.md](SECURITY.md) §7).
