...
- **Trust is anchored on the user, and never on certifications merely existing.** *Valid* and *invalid* always describe a signature; they never describe a contact. The app asserts exactly two kinds of trust, kept visibly apart and never merged into one badge: the user's own fingerprint verification ("Verified by You"), and — one hop out, no further — a certification made by a contact whose fingerprint the user personally verified ("Vouched by *contact*"). A certification from an unverified signer carries no weight and says so; a certification the user made themselves records their own act rather than someone else's word; a key's self-signature is not a certification at all and cannot be saved as one. Colour follows the claim: green belongs to the user's own verification alone, and a signature that merely verifies gets no colour, because "this signature is valid" must never read as an endorsement.
- **Vouching weight is derived live, never stored.** It is recomputed from the current set of verified contacts on every read, so withdrawing a verification removes the weight of everything anchored on it at once — there is no cached verdict to go stale. Revoking a signer's key withdraws its vouches too; a signer's key merely *expiring* does not retract what it already said. The web is computed only over certificates already imported: no discovery, no refresh, no key servers.
