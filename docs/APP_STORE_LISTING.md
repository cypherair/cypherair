...
### What's New — 1.5.2 template (4000 chars max)

```
• Three new · High key families: Portable Modern · High (Ed448), Portable Post-Quantum · High, and Device-Bound Post-Quantum · High (RFC 9980, ML-KEM-1024) — the strongest tiers, joining the existing six for nine key families in all.
...
• Secure Enclave custody: Device-Bound Legacy and Device-Bound Modern keys whose private halves never leave this device.
...
```
• 三个全新的 · 高强度密钥家族：便携现代 · 高强度（Ed448）、便携后量子 · 高强度与设备绑定后量子 · 高强度（RFC 9980，ML-KEM-1024）——最强等级，与原有六个家族共同构成九个密钥家族。
• 后量子密钥（RFC 9980）：可生成便携后量子与设备绑定后量子密钥家族，并直观查看消息是否量子安全。
• 安全隔区托管：设备绑定旧版与设备绑定现代密钥的私钥永不离开本机。
