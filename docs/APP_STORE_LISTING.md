...
Positioning in one line: **CypherAir X is the enhanced edition of CypherAir** —
it adds the three Device-Bound (Secure Enclave) families and Portable
Post-Quantum on top of CypherAir's two portable families, while keeping the
fully offline, zero-network privacy model.
...
```
CypherAir X is the enhanced edition of CypherAir — a fully offline OpenPGP encryption tool. It never connects to the internet: no telemetry, no accounts, no key servers. Your keys and messages stay on your device.

WHY CYPHERAIR X
Beyond the Portable Compatible and Portable Modern key families of CypherAir, CypherAir X adds four more:
• Device-Bound Compatible — P-256 keys held in the Secure Enclave
...
### What's New — 1.5.0 template (4000 chars max)

```
• Post-quantum keys (RFC 9980): generate Portable and Device-Bound Post-Quantum key families and see at a glance when a message is quantum-safe.
• Secure Enclave custody: Device-Bound Compatible and Device-Bound Modern keys whose private halves never leave this device.
• New welcome tour, unified security settings, redesigned About and App Icon pages.
```
...
```
CypherAir X 是 CypherAir 的增强版——一款完全离线的 OpenPGP 加密工具。它永不联网：没有遥测、没有账户、没有密钥服务器。你的密钥和消息始终留在你的设备上。

为什么选择 CypherAir X
在 CypherAir 的便携兼容与便携现代密钥家族之外，CypherAir X 新增四个密钥家族：
• 设备绑定兼容——私钥保存在安全隔区中的 P-256 密钥
• 设备绑定现代——私钥保存在安全隔区中的 RFC 9580 v6 密钥
...
```
• 后量子密钥（RFC 9980）：可生成便携后量子与设备绑定后量子密钥家族，并直观查看消息是否量子安全。
• 安全隔区托管：设备绑定兼容与设备绑定现代密钥的私钥永不离开本机。
• 全新欢迎导览、统一的安全设置、重新设计的“关于”与“App 图标”页面。
```
...
```
CypherAir X is the paid, enhanced edition of CypherAir by the same developer. What is new relative to CypherAir:

- Four additional key families on top of CypherAir's two, for six total. Three Device-Bound families generate and hold private keys inside the Apple Secure Enclave (non-exportable by design), and the Post-Quantum families implement RFC 9980 to protect messages against future quantum-computer attacks.
- Redesigned welcome tour, unified security settings, and new About and App Icon pages.
...
- iPhone 6.9" set — Home, Encrypt, key generation (six families), About.
- iPad 13" set.
- Mac set (wide layout).
