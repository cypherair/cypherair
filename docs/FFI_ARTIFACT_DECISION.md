...
- **Artifact.** `PgpMobile.xcframework` is a **static-library** XCFramework: five slices — the iOS, macOS, and visionOS **device** libraries are lipo-combined `arm64+arm64e` universals (built from both the `aarch64-apple-*` and `arm64e-apple-*` Rust triples against the pinned stage1; the release manifest validator requires the `arm64e` architectures), plus the `arm64`-only iOS and visionOS simulator libraries — each slice carrying `libpgp_mobile.a` plus a `Headers/` directory with `pgp_mobileFFI.h` and a `module.modulemap`. It is a build product, git-ignored (`*.xcframework/`), rebuilt from source against the pinned arm64e stage1; a project build-phase guard fails the build if it is missing. The release-grade binary and its SDK channels (Edge/Drill/Stable) are owned by [RELEASE.md](RELEASE.md) §3 and §5 and not restated here.
...
3. **Upstream UniFFI packaging change.** A UniFFI release (beyond the pinned 0.32.x) that changes the generated module/import name, ships a framework-shaped packaging path, or changes the modulemap/header layout enough to break the current sync — re-evaluate both the shape and the `pgp_mobileFFI` vs `PgpMobileFFI` name at that boundary. Checked at the 0.31.2 → 0.32.0 bump: the generated header and both modulemaps are byte-identical, so the `pgp_mobileFFI` module name and layout carry forward unchanged.
