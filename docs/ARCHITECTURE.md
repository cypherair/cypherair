...
- `ContactsLegacyMigrationSource` is the only production path that reads legacy flat `Contact` records, and it does so only to build the first protected-domain snapshot from active old-install `Documents/contacts` files. `ContactsCompatibilityMapper` maps that migration input into the protected-domain snapshot; it is not a runtime projection path.
