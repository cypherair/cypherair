...
| `ContactService` | Single app/UI-facing Contacts facade for availability, public-key import/update, person/key queries, tags, recipient lists, merge, verification state, certification projection/artifact access, protected-domain mutation rollback, and relock cleanup |
...
- `ContactService` is the only app/UI-facing Contacts facade. It owns Contacts availability, person/key query APIs, tag and recipient-list APIs, merge/preferred-key mutations, verification state, certification projection/artifact access, mutation rollback, migration/quarantine cleanup warnings, and relock cleanup.
...
- `ContactsDomainRepository` owns Contacts schema serialization, compatibility projections, and runtime scratch/search/signer state clearing. Current protected payloads include person-centered identities, per-key public certificate records, manual verification state, key usage state, certification projection/artifacts, tags, and recipient lists.
- `ContactsSearchIndex` is an unlocked-only in-memory index for Contacts and Encrypt search. Search indexes, screen filters, recipient selection, and signer lookup helpers must not persist outside the protected Contacts payload.
...
- current Phase 1-7 ProtectedData work is implemented, including ordinary-settings, self-test export-only, and temporary/export/tutorial hardening; the protected `contacts` domain is the authoritative source for current Contacts state after app unlock
...
│       ├── contacts/                        → Protected Contacts snapshot envelopes
