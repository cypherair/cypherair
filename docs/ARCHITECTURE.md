...
| `KeyManagementService` | Key generation across all six families — Portable Compatible (Cv25519/RFC 4880), Portable Modern (Cv448/RFC 9580), Portable Post-Quantum (RFC 9980 composite), Device-Bound Compatible/Modern (Secure Enclave custody P-256 v4/v6), Device-Bound Post-Quantum (split custody via `CompositeCustodyRouterContext`) — plus import, export, expiry modification, revocation and selective-revocation export, and selector discovery. Implementation helpers live in `KeyManagement/`. |
...
| `SecureEnclaveCompositeHandleStore` | Lifecycle boundary for the enclave-resident ML-DSA-65/ML-KEM-768 component blobs of Device-Bound Post-Quantum identities (store/load/locate/delete by handle-set id and public binding) |
| `SecureEnclaveCompositeClassicalComponentStore` | Fixed-access `CAPKEV1` envelope custody for the Ed25519+X25519 classical components (exempt from mode-switch re-wrap) |
| `SecureEnclaveCompositeOperations` | In-enclave ML-DSA signing and ML-KEM decapsulation primitives consumed by the composite provider bridges |
