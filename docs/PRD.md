...
- **Decryption:** the App accepts and decrypts both v4 and v6 messages regardless of the user's own key family.
- **Multiple keys:** a user may hold keys of different families (e.g. a Portable Legacy key for GnuPG contacts and a post-quantum key for the future-proof ones). One key is designated "Default." The family is immutable after generation — switching means generating a new key.

### 3.4 Apple Secure Enclave Custody (Device-Bound key families)

Secure Enclave custody is the hardware-backed private-key custody mode behind the Device-Bound families — a custody model, not an algorithm suite. Private-key operations happen inside the Secure Enclave and the long-term private scalar never enters CypherAir's Swift or Rust plaintext memory. Device-Bound Post-Quantum extends this as **split custody**: the ML-DSA-65/ML-KEM-768 components (or ML-DSA-87/ML-KEM-1024 for the · High tier) live in the enclave, the Ed25519/X25519 classical components (Ed448/X448 for · High) are sealed under a fixed-access envelope, and every composite signature or decryption requires an in-enclave operation — the classical component alone can do neither. Full custody contract and evidence: [SECURE_ENCLAVE_CUSTODY](SECURE_ENCLAVE_CUSTODY.md); split-custody rationale: [POST_QUANTUM](POST_QUANTUM.md).
...
- **Cosmetic privacy cover:** whenever the app is not foreground-active, the shield window shows its opaque privacy face — the same window-level surface family as the lock screen, minus any authentication affordance — above all app content, open sheets and covers included (multitasking snapshot, shoulder-surfing). Purely visual; no coupling to authentication.
