...
> **Version:** Draft v0.3
> **Status:** Draft implementation-prep checklist, updated with Contacts PR5 person-centered model coverage notes.
...
Contacts PR5 then moves ordinary Contacts UI and Encrypt behavior to person-centered `ContactIdentity` semantics, adds explicit merge and preferred/additional/historical key management, and keeps flat `Contact` rows as compatibility projection only. Remaining follow-on work still includes `.cypherair-contacts` packages, certification projection persistence/UX, search, tags, recipient-list UI, and organization workflows.
