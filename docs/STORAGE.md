...
- **Self-test reports** — held in process memory, export-only, never persisted; saving one transits the same erased `tmp/` staging every export uses.
...
- **Temporary artifacts** live under verified-file-protection `tmp/` paths and are swept **once per launch, and never at termination**; the sweep erases only what the running session does not own. Export handoffs are still written flat into the `tmp/` root and swept by filename prefix, and each export surface stages through a store of its own — so a handoff staged while the launch sweep is running is not in that sweep's live set and can be erased under the open picker. **Roadmap:** bring the flat export handoffs under the owned root and sweep by ownership rather than filename, which retires both.
- **No `tmp/` path CypherAir creates is named after what it holds.** Every one is a UUID or a fixed component; the name a save is offered under ([PRODUCT.md](PRODUCT.md) §5) is carried on the artifact in memory and never written to a path the app controls. A `tmp/` listing is readable by more than the app, and a filename states what a protected file's bytes do not. **The guarantee stops at the app's own paths:** what the system's export machinery stages between the app and the document picker is created, named, and removed outside CypherAir, and the launch sweep's prefix match does not reach it.
