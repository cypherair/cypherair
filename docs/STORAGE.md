...
- **No temporary path is named after what it holds.** Every one is a UUID or a fixed component; the name a save is offered under ([PRODUCT.md](PRODUCT.md) §5) is carried on the artifact in memory and never written to disk. A `tmp/` listing is readable by more than the app, and a filename states what a protected file's bytes do not.
