...
- **`Documents/Inbox` is swept as well, and never spared.** It is the one swept root outside `tmp/`, and the only one the app does not create: the system fills it when another app hands CypherAir a document to open. Nothing a running session uses lives there — an opened document is read and dropped, or moved under the owned `tmp/opened/` root — so anything present is a previous session's abandoned copy, and the live-artifact sparing that protects the `tmp/` roots deliberately does not apply. A document opened *in place* is never touched: that file is the reader's, and only inbox copies are ever erased.
