...
- **Domain payloads are sealed snapshots, one file per domain.** One AES-256-GCM envelope under the domain's derived key, with the domain and schema bound as authenticated data, written to a temporary file and renamed into place. No generations, no registry, no watermark. Contacts is such a domain like every other: it has no database and no sidecar files.
- **Damage has one destination.** A sealed root without its domain files, or a domain file that is missing or fails to authenticate, is the integrity-failure state ([SECURITY.md](SECURITY.md) §5) — never a bootstrap into empty state and never a default value.
...
- **Every portable private key is one self-contained envelope row**, sealed against the identity wrapping key's public key with a fresh ephemeral key; the row holds no enclave key of its own. An expiry change replaces the row's value in place; there are no pending rows.
