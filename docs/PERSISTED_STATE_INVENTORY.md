...
| Secure Enclave composite (post-quantum) private-key blob rows | Keychain `kSecClassGenericPassword` rows, service `com.cypherair.v1.secure-enclave-composite.<role>` for the base tier and `com.cypherair.v1.secure-enclave-composite.high.<role>` for the · High tier, plus account `<random-handle-set-id>`; two rows per Device-Bound Post-Quantum identity holding the CryptoKit Secure Enclave `dataRepresentation` blob (`kSecValueData`) — ML-DSA-65 / ML-KEM-768 for the base tier, ML-DSA-87 / ML-KEM-1024 for Device-Bound Post-Quantum · High — plus the component public key (`kSecAttrGeneric`) for non-prompting locate. Data-protection keychain, this-device-only, non-synchronizable; the blob is useless off-device. Handle-set ids are Security-private local locators, not stored in `key-metadata`, logs, UI, Rust, or exports. Reset All Local Data inventories and deletes app-owned composite rows and validates none remain using sanitized counts/categories only. Guarded device test (`DeviceSecureEnclaveCompositeCustodyTests`) validates real hardware create/load/locate/delete and split-custody operations. | `private-key-material exception` | Secure Enclave composite handle store (campaign #567 Phase 3) | Implemented production boundary with local reset cleanup and guarded device evidence; user-reachable on Secure Enclave hardware via Device-Bound Post-Quantum and Device-Bound Post-Quantum · High generation | implemented |
