...
| Unsupported legacy contacts public-key files | `Documents/contacts/*.gpg` and historical `Documents/contacts.quarantine/*.gpg` | `unsupported legacy` | Contacts support cutoff | Not read, migrated, quarantined, or reset-cleaned; existing files may remain on disk but are no longer treated as CypherAir app state | unsupported |
| Unsupported legacy contacts metadata | `Documents/contacts/contact-metadata.json` and historical quarantine copy | `unsupported legacy` | Contacts support cutoff | Not read, migrated, quarantined, or reset-cleaned; existing files may remain on disk but are no longer treated as CypherAir app state | unsupported |
