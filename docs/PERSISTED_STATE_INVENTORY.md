...
This inventory tracks current shipped state plus pending classified surfaces. It is not a roadmap narrative. Current Contacts persisted-state classification lives in this inventory; user-visible behavior and durable technical/security contracts live in [PRD](PRD.md), [TDD](TDD.md), [ARCHITECTURE](ARCHITECTURE.md), [SECURITY](SECURITY.md), and [TESTING](TESTING.md). Archived Contacts planning documents are historical context only.
...
| Contacts payload | `Application Support/ProtectedData/contacts/`; protected `ContactsDomainSnapshot` containing PR5 person/key state (`ContactIdentity`, `ContactKeyRecord`, stored public certificate `publicKeyData`, manual verification, key usage state), PR6 certification state (projection, saved valid certification signature artifacts, canonical binary signature bytes, digest, selector, signer metadata, export filename), and PR8 organization state (tags, tag memberships, recipient lists) | `protected-after-unlock` | Phase 8 / Contacts PR4-PR8 | Implemented through Contacts PR8 | implemented |
| Contacts runtime-only search/filter/selection state | In memory only: `ContactsSearchIndex`, screen search/filter values, tag filters, recipient selection, and pending route state | `ephemeral-with-cleanup` | Contacts PR8 runtime exception | Implemented; cleared with relock, content clear, or screen lifecycle as applicable | n/a, not persisted |
| Legacy contacts public-key files | `Documents/contacts/*.gpg` and `Documents/contacts.quarantine/*.gpg` | `legacy cleanup-only` | Phase 8 / Contacts PR4 | Cutover source and quarantine cleanup only; inactive after protected open; does not carry PR6/PR8 production state | cleanup only |
| Legacy contacts metadata | `Documents/contacts/contact-metadata.json` and quarantine copy | `legacy cleanup-only` | Phase 8 / Contacts PR4 | Cutover source and quarantine cleanup only; inactive after protected open; does not carry PR6/PR8 production state | cleanup only |
...
| `tmp/export-*` | App temporary directory `tmp/export-<UUID>-<sanitized filename>`, including explicit PR6 certification-signature export handoff files before user-selected export/share leaves app custody | `ephemeral-with-cleanup` | Phase 7 PR 4 / Contacts PR6 explicit export boundary | Implemented | implemented |
...
- do not treat archived Contacts planning documents as active current-state update targets; update them only when deliberately preserving historical context or archive metadata
...
For Phase 8 Contacts PR5/6/8, this inventory is the authoritative persisted-state classification. Search indexes, screen filters, and recipient selections are runtime state only; they must not become a second persisted source of truth outside the protected `contacts` payload.
