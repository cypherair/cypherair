...
| `tmp/export-*` | App temporary directory `tmp/export-<UUID>-<sanitized filename>`, including explicit certification-signature export handoff files before user-selected export/share leaves app custody | `ephemeral-with-cleanup` | Phase 7 temporary artifact / certification-signature explicit export boundary | Implemented | implemented |
