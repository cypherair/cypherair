...
| Shared app-data root-secret envelope row | Keychain default account service `com.cypherair.protected-data.shared-right.v1`; LA-gated binary-plist `CAPDSEV2` envelope containing the wrapped 32-byte ProtectedData root secret. It unwraps only through the separate ProtectedData device-binding key row and the existing app-session Keychain authentication gate. Reset All Local Data deletes it by exact service cleanup. | `framework-bootstrap` | Shared app-data bootstrap | Implemented with SE device binding; currently paired with a separate device-binding key row | implemented |
| ProtectedData device-binding key row | Keychain default account service `com.cypherair.v1.protected-data.device-binding-key`; ProtectedData-only P-256 Secure Enclave key `dataRepresentation` used to open the `CAPDSEV2` root-secret envelope after the app-session Keychain gate succeeds. This is the current separate second row; single-row modernization is tracked for evaluation outside this inventory. Reset All Local Data deletes it by `com.cypherair.v1.` prefix sweep plus exact postcondition cleanup. | `framework-bootstrap` | ProtectedData SE device-binding key | Implemented | implemented |
