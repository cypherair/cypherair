...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Contacts legacy file runtime and protected-domain cutover | `Sources/Services/ContactService.swift`, `Sources/Services/ContactRepository.swift`, `Sources/Services/ContactsLegacyMigrationSource.swift`, `Sources/Services/ContactsCompatibilityMapper.swift`, `Sources/Security/ProtectedData/ContactsDomainSnapshotCodec.swift`, `Sources/App/AppContainer.swift` | Retain But Isolate | Production opens the ProtectedData `contacts` domain after authentication and uses the protected domain as the only runtime source of truth. Legacy `Documents/contacts` remains only as the first protected-domain creation migration source plus quarantine/reset cleanup. Protected open/create failure no longer falls back to legacy runtime state. | Keep tests that prove legacy contacts migrate once, corrupt legacy input leaves active source intact for retry, quarantine is not used as fallback, protected state wins after cutover, protected mutations do not write active legacy files, relock clears runtime state, and reset removes legacy files. | Keep legacy source reads inside `ContactsLegacyMigrationSource.makeInitialSnapshot()` and cleanup methods. Keep `ContactService` as the app-facing protected-domain facade. | The retained old-install migration source can be removed only after a separate support cutoff confirms supported installs no longer need flat Contacts migration. | High: contacts carry public certificates and trust/verification state; wrong cleanup can lose contacts or reactivate plaintext state. |
| Legacy flat contact key-replacement flow | `Sources/Services/ContactService.swift`, `Sources/App/Contacts/Import/ContactImportWorkflow.swift` | Completed Cleanup Candidate | Completed: the app-facing legacy key-replacement request/result path was removed. Protected-domain Contacts import related different-fingerprint keys as separate identities and uses candidate/merge behavior instead of replacement confirmation. | Legacy replacement detection and confirm-replacement tests were removed or rewritten to candidate/merge outcomes. Source-audit coverage blocks reintroducing the legacy runtime vocabulary in production sources. | No further action for the deleted key-replacement runtime path. Keep current candidate/merge behavior coverage. | Completed by Phase 3 PR3C. | Medium: mostly public-key/contact UX; current risk is regression prevention. |
...
| Legacy Contacts flat-runtime tests | `ContactServiceTests` migration/cutover/recovery cases and test-only protected-domain open helpers | Completed Rewrite / Retain Migration | Keep migration/cutover tests. Tests that only preserved runtime fallback, legacy availability, or legacy replacement outcomes were removed or rewritten to protected-domain state and retained migration behavior. |
