...
- Phase 1A removed `ContactService.availableContacts(matchingKeyIds:)`,
  ...
  check. `legacyPublicKeysForRecipientFingerprints(_:)` and the
  fingerprint-recipient compatibility layer remain for the Phase 2 migration.
...
- Phase 2 first PR completed PR 2A/2B/2C/2F: service and tutorial tests now
  use contact-ID recipient APIs, and `EncryptView.Configuration` no longer
  exposes route/tutorial fingerprint preselection. The `EncryptionService`
  fingerprint overloads, `ContactService.legacyPublicKeysForRecipientFingerprints(_:)`,
  and legacy resolver remain for PR 2D/2E.
...
| Fingerprint recipient encryption compatibility | `Sources/Services/EncryptionService.swift`, `Sources/Services/ContactRecipientResolver.swift`, `Sources/Services/ContactService.swift` | Retain But Isolate, with cleanup candidates inside it | Current production UI passes contact IDs. `recipientFingerprints` overloads and `legacyPublicKeysForRecipientFingerprints` remain for older callers. `publicKeysForRecipientFingerprints(_:)` has already been removed. | Phase 2 first PR migrated `EncryptionServiceTests`, `StreamingServiceTests`, `DecryptionServiceTests`, `PasswordMessageServiceTests`, and `TutorialSessionStoreTests` to contact-ID APIs. `ContactServiceTests` retains explicit resolver compatibility coverage until PR 2E. | Remove `EncryptionService` recipient-fingerprint overloads in PR 2D, then remove `ContactService.legacyPublicKeysForRecipientFingerprints(_:)` and the legacy resolver in PR 2E. | When PR 2D/2E are implemented and no production or test references require the compatibility layer. | Medium: encryption behavior is security-sensitive, but this layer is recipient lookup compatibility, not cryptographic format selection. |
| No-caller ContactService helpers | `Sources/Services/ContactService.swift` | Completed Cleanup Candidate | Phase 1A removed `availableContacts(matchingKeyIds:)`, `publicKeys(for:)`, and deprecated `publicKeysForRecipientFingerprints(_:)` after confirming no direct production or test callers. | No direct test callers were found for these exact symbols. Related fingerprint-recipient tests cover the broader compatibility path, not these helpers directly. | No further Phase 1A action. Keep `legacyPublicKeysForRecipientFingerprints(_:)` for Phase 2. | Completed. | Low: the retained recipient compatibility layer remains untouched. |
...
- Phase 1A completed the no-caller ContactService helper cleanup:
  ...
  moved broader fingerprint-recipient tests to contact-ID APIs; PR 2D/2E remain
  for deleting the compatibility layer itself.
...
| Fingerprint-recipient tests | `EncryptionServiceTests`, `StreamingServiceTests`, `DecryptionServiceTests`, `PasswordMessageServiceTests`, `TutorialSessionStoreTests` formerly using `recipientFingerprints`; `ContactServiceTests` explicit resolver compatibility coverage | Mostly completed Rewrite Candidate | Service and tutorial coverage now uses contact-ID recipient APIs. Keep only explicit resolver compatibility coverage until PR 2E removes that surface. |
