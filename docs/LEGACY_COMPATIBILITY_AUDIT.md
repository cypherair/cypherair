...
- Phase 1A completed the no-caller ContactService helper cleanup:
  ...
  `publicKeysForRecipientFingerprints(_:)` were removed. Broader
  fingerprint-recipient tests still need to move away from
  `recipientFingerprints` overloads before the whole compatibility layer is
  removed.
