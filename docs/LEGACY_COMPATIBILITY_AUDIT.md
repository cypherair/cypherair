...
For detailed verification/decryption specifically, the cleanup direction is
deletion of the non-detailed legacy surface after valuable behavior coverage is
migrated to detailed APIs. It is not a plan to retain legacy folded-summary
compatibility tests indefinitely.
...
| Swift/Rust detailed verification cutover and legacy verification/decrypt surface | `pgp-mobile/src/signature_details.rs`, `pgp-mobile/src/verify.rs`, `pgp-mobile/src/decrypt.rs`, `pgp-mobile/src/streaming.rs`, `Sources/Models/DetailedSignatureVerification.swift`, `Sources/Models/SignatureVerification.swift`, `Sources/Services/SigningService.swift`, `Sources/Services/DecryptionService.swift`, `Sources/Services/PasswordMessageService.swift`, `Sources/Services/SelfTestService.swift` | Cleanup Candidate after detailed migration gates | UI verify/decrypt screen models already use detailed APIs. Legacy-returning `SigningService` and `DecryptionService` facades internally call detailed engine APIs, then fold back to `SignatureVerification`. `SelfTestService` still directly calls simple `engine.decrypt` and `engine.verifyCleartext`. `PasswordMessageService` is not a simple verification API, but it folds detailed password decrypt fields into `SignatureVerification`. | Migrate valuable behavior coverage in `SigningServiceTests`, `DecryptionServiceTests`, `StreamingServiceTests`, `GnuPGInteropTests`, `DeviceMIETests`, `FFIIntegrationTests`, password-message tests, and self-test tests to detailed APIs. Delete tests that only preserve simple API compatibility, detailed-vs-legacy equivalence, legacy folded-summary bridge behavior, or legacy-only cancel/error paths. | Use Phase 3A/3B/3C sequencing: close production residual simple API use first, migrate important behavior scenarios to detailed APIs, then delete the non-detailed Rust/UniFFI exports, generated Swift engine methods, simple-result records, Swift/app `SignatureVerification` primary-result facades, and legacy-only tests. | After `SelfTestService` and all required behavior gates are detailed, focused reference checks find no simple verification/decrypt calls, and no legacy folded-summary compatibility tests remain. | High: verification status semantics affect user trust decisions; deletion must preserve the valuable behavior coverage while intentionally removing legacy summary compatibility. |
...
- Raw first-match User ID FFI methods:
  ...
  contract is retired.
- Non-detailed verification/decrypt surface after Phase 3 migration gates:
  ...
  Swift/app APIs that expose `SignatureVerification` as the primary
  verification result.
...
| Simple FFI / legacy verification tests | `SigningServiceTests`, `SigningServiceDetailedResultTests`, `DecryptionServiceTests`, `StreamingServiceTests`, `GnuPGInteropTests`, `DeviceMIETests`, `FFIIntegrationTests`, `SelfTestServiceTests` | Cleanup Candidate after detailed migration gates | Migrate important behavior to detailed APIs, including Device MIE, GnuPG interop, file streaming artifact cleanup, High Security decrypt auth failure, and normal valid/tamper/unknown-signer/contact-resolution scenarios. Delete tests that only verify simple API compatibility or legacy bridge equivalence; do not retain explicit legacy compatibility tests for this surface. |
...
- GnuPG interoperability and Profile A compatibility language.
  Keep the behavior coverage, but Phase 3 should assert it through detailed
  APIs instead of relying on simple verification/decrypt APIs.
