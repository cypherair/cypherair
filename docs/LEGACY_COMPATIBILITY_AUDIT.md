...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Contacts legacy file runtime and protected-domain cutover | `Sources/Services/ContactService.swift`, `Sources/Security/ProtectedData/ContactsDomainSnapshotCodec.swift`, `Sources/Security/ProtectedData/ContactsDomainStore.swift`, `Sources/App/AppContainer.swift` | Completed Cleanup Candidate | Completed by Phase 3 PR3D: production opens the ProtectedData `contacts` domain after authentication and uses the protected domain as the only runtime source of truth. Legacy `Documents/contacts` files are no longer read, migrated, quarantined, or reset-cleaned. First protected-domain creation starts from `ContactsDomainSnapshot.empty()`. | Keep tests that prove empty protected-domain creation, protected mutations across reopen, corrupt/missing protected state recovery, schema v1-to-v2 protected snapshot migration, and relock/framework reset cleanup. Tests that only proved flat-file migration, quarantine retry, corrupt legacy source handling, or reset cleanup of legacy Contacts files were removed. | Keep `ContactService` as the app-facing protected-domain facade and `ContactsDomainStore` as the sole persistence owner. Source-audit guardrails now block reintroducing flat `Contact`, `ContactRepository`, `ContactsLegacyMigrationSource`, or `ContactsCompatibilityMapper`. | Completed by Phase 3 PR3D support cutoff. | Medium: contacts carry public certificates and trust/verification state; current risk is protected-domain recovery and regression prevention, not legacy file cleanup. |
...
| --- | --- | --- | --- |
| Real migration/recovery safety tests | `ProtectedDataFrameworkTests`, `LocalDataResetServiceTests`, private-key control and key metadata migration tests, Contacts protected-domain recovery tests | Retain But Isolate | Keep. Regroup around migration/recovery components when code is split. |
