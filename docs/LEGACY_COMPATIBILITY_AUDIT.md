...
| Legacy flat contact key-replacement flow | `Sources/Services/ContactService.swift`, `Sources/App/Contacts/Import/ContactImportWorkflow.swift` | Retain But Isolate | `.legacyKeyReplacementDetected` is documented as legacy flat Contacts behavior through `ContactImportResult`. Protected-domain Contacts import related different-fingerprint keys as separate identities and reject legacy replacement. UI still handles the legacy result defensively through `ContactLegacyKeyReplacementRequest`. | `ContactServiceTests` covers `legacyKeyReplacementDetected` and confirm-replacement behavior. These should be split into legacy-runtime tests and removed when the legacy runtime exits. | Move key-replacement behavior behind a legacy Contacts runtime adapter. Keep protected-domain import behavior separate. | When `openLegacyCompatibilityForTests` and the legacy flat runtime are retired or no longer part of supported data migration. | Medium: mostly public-key/contact UX, but deletion could break old contact replacement tests before the runtime is removed. |
...
| Legacy Contacts flat-runtime tests | `ContactServiceTests` cases around `openLegacyCompatibilityForTests`, `.availableLegacyCompatibility`, and `.legacyKeyReplacementDetected` | Mixed | Keep migration/cutover tests. Remove or isolate tests that only preserve legacy flat runtime after the runtime is retired. |
