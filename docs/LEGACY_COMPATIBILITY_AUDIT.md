...
- Phase 4A-D completed selector contract confirmation and test migration:
  Swift service APIs remain selector-backed despite their existing names,
  `FFIIntegrationTests` and Rust tests now use selector APIs for User ID
  certification/revocation/verification coverage, and first-match-only duplicate
  compatibility tests were removed. Raw first-match exports remain pending PR
  4E deletion and generated binding refresh.
...
| Raw User ID first-match FFI APIs | `pgp-mobile/src/cert_signature.rs`, `pgp-mobile/src/keys/revocation.rs`, `pgp-mobile/src/lib.rs`, `Sources/Services/CertificateSignatureService.swift`, `Sources/Services/KeyManagement/SelectiveRevocationService.swift` | Pending Export Cleanup after Phase 4A-D | Production Swift services use selector-based APIs for certification and selective revocation. Raw first-match FFI APIs remain exported only pending PR 4E export/helper deletion; tests no longer keep first-match compatibility alive. | `FFIIntegrationTests` and Rust tests now cover selector-based certification/revocation/verification behavior. First-match-only duplicate compatibility tests were removed, and direct raw first-match test calls are gone. | Remove raw FFI exports and internal raw helpers in PR 4E, then regenerate bindings through the normal workflow. Keep selector APIs as the current production contract. | When generated bindings and downstream callers no longer need raw first-match methods; PR 4E owns that deletion. | Medium to High: duplicate User ID selection is correctness-sensitive; raw deletion must not remove selector coverage. |
...
- Raw first-match User ID FFI methods:
  ...
  equivalents. Phase 4A-D rewrote selector-value coverage and removed raw
  first-match tests; PR 4E remains for export/helper deletion and binding
  regeneration.
...
| Raw User ID first-match tests | `FFIIntegrationTests`, `pgp-mobile/tests/certification_binding_tests.rs`, `pgp-mobile/tests/revocation_construction_tests.rs`, `pgp-mobile/tests/selector_discovery_tests.rs` | Completed Rewrite / Pending Export Cleanup | Selector coverage is preserved through by-selector APIs. Direct raw first-match calls and first-match-only duplicate compatibility tests were removed in Phase 4A-D; PR 4E should remove the raw exports/helpers and refresh generated bindings. |
