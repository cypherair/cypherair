...
Phase 6A adds macOS unit coverage for hidden/test decrypt routing, key-agreement handle lookup, Security bridge shared-secret derivation and failure mapping, FFI adapter error mapping, external P-256 key-agreement containment, and proof that `DecryptionService` is not a consumer yet.
