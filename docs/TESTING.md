...
**What they cover:** Sequoia PGP operations in isolation — key generation (both profiles), recipient-key encrypt/decrypt, password/SKESK encrypt/decrypt, sign/verify, armor encode/decode, error mapping, and S2K (Iterated+Salted and Argon2id) export/import. They also cover the Rust-only Phase 2A external P-256 signer proof: v4/v6 public-only certificate construction, cleartext and detached signing through the shared Sequoia signing stream, selector/public-certificate validation, signer failure without software secret-certificate fallback, typed callback cancellation/category propagation, and fail-closed hash/digest/signature-shape/public-binding checks. Phase 2B adds a Rust-only external P-256 ECDH/session-key proof: v4 public-only candidates decrypt SEIPDv1/MDC messages, v6 candidates decrypt SEIPDv2/AEAD messages, signed encrypted messages preserve signature status, public-certificate validation and selector discovery accept the candidates, ECDH failure does not fall back to secret-certificate material, and unsupported key/ciphertext/shared-secret shapes fail closed. Phase 2C closes the Rust negative matrix for external-operation wrong roles, wrong public bindings, session-key validation failures, and tampered SEIPDv1/MDC plus SEIPDv2/AEAD payload hard-fail after session-key acceptance. Phase 5B adds the runtime cleartext external-signer API matrix: v4/v6 public-only P-256 cleartext signatures verify, callback cancellation and sanitized categories survive UniFFI mapping, malformed `r/s`, wrong digest, wrong public key, non-P-256 or wrong-role certificates, fingerprint mismatch, and no secret-certificate fallback all fail closed. Phase 5C adds the runtime text sign-plus-encrypt external-signer API matrix: v4/v6 public-only P-256 signed encrypted messages decrypt and verify, cancellation and typed callback categories survive encrypt finalization, secret signing certificates, wrong fingerprints, non-P-256 or wrong-role certificates, malformed signatures, wrong digests, wrong public keys, and external failures fail closed without secret-certificate fallback. Phase 5D adds the runtime password-message external-signer API matrix: v4/v6 public-only P-256 signed password messages decrypt and verify for armored and binary outputs, cancellation and typed callback categories survive finalization, secret signing certificates, wrong fingerprints, non-P-256 or wrong-role certificates, malformed signatures, wrong digests, wrong public keys, and external failures fail closed without secret-certificate fallback. Phase 5E adds the runtime detached file external-signer API matrix: v4/v6 public-only P-256 detached file signatures verify, progress and callback cancellation are preserved, sanitized categories survive finalization, secret signing certificates, wrong fingerprints, non-P-256 or wrong-role certificates, malformed signatures, wrong digests, wrong public keys, and external failures fail closed without secret-certificate fallback. Phase 5F adds the runtime streaming file encrypt-plus-sign external-signer API matrix: v4/v6 public-only P-256 signed file messages decrypt and verify, SEIPDv1/SEIPDv2 selection honors recipient and encrypt-to-self downgrade rules, progress and callback cancellation are preserved, sanitized categories survive finalization, secret signing certificates, wrong fingerprints, non-P-256 or wrong-role certificates, malformed signatures, wrong digests, wrong public keys, and external failures fail closed without secret-certificate fallback. Phase 5G adds the runtime public-only expiry mutation external-signer API matrix: v4/v6 P-256 expiry set/remove mutations preserve fingerprint and key version, update expiry metadata, emit verifiable binding signatures, preserve cancellation and typed callback categories, and reject secret certificates, wrong fingerprints, non-P-256 or wrong-role certificates, malformed signatures, wrong digests, wrong public keys, and external failures without secret-certificate fallback. Phase 5H adds the runtime public-only selective revocation external-signer API matrix: v4/v6 P-256 subkey and User ID revocation signatures revoke the selected target after insertion, external signatures use SHA-256 while software builder defaults are preserved, key-level revoked or unresolved public certs fail before callback, expired-but-not-revoked certs remain usable, primary-only signer selection rejects signing-capable subkey fingerprints, and cancellation, typed callback categories, malformed responses, wrong digests, wrong public keys, selector misses, secret input, non-P-256, wrong-role, and no-fallback cases all fail closed. Phase 5I adds the runtime public-only User ID certification external-signer API matrix: v4/v6 P-256 certifications verify for all certification kinds, external signatures use SHA-256 while software certification defaults are preserved, key-level revoked or unresolved public certs fail before callback, expired-but-not-revoked certs remain usable, primary-only signer selection rejects signing/certification subkey fingerprints and primaries missing certification capability, and cancellation, typed callback categories, malformed responses, wrong digests, wrong public keys, selector misses, secret input, non-P-256, wrong-role, and no-fallback cases all fail closed.
Phase 5G follow-up coverage explicitly verifies that software and public-only external expiry mutation refresh explicit transport/ECDH subkey validity bindings, can extend or remove expiry after the local key is already expired, and that a signing-capable subkey fingerprint cannot be used as the public-only expiry signer. Swift coverage also verifies Secure Enclave expiry writeback merges against the current catalog identity and does not resurrect metadata deleted during the external-signing round trip.
Phase 5J does not add Rust/UniFFI APIs; it closes Phase 5 by auditing that the existing Phase 5B-5I runtime matrices remain the only signing-class Secure Enclave signer-route consumers.
Phase 6A adds the runtime public-only external P-256 key-agreement API matrix: v4/v6 P-256 encrypted messages decrypt through the callback, signed messages preserve verification folding, and Rust rejects secret certificates, wrong fingerprints, signing-role/non-P-256/wrong-role certificates, malformed or zero shared secrets, wrong public bindings, session-key validation failures, payload tampering, callback cancellation/category failures, and no secret-certificate fallback. Follow-up coverage hard-aborts invalid callback responses before later PKESKs, verifies Swift shared-secret handoff zeroization/audit coverage, and fixes the runtime bridge's shape-valid off-curve point rejection boundary.
Phase 6D does not add Rust/UniFFI APIs; it closes Phase 6 by adding mixed-recipient file decrypt, repeated message/file decrypt that stays consistent and leaves no residual `.tmp` scratch artifact, and no-partial-plaintext hard-fail on a tampered mixed-recipient payload over the existing Phase 6A-6C key-agreement decrypt routes.
...
**What they cover:** Services layer logic, model validation, key operation failure-category vocabulary, error message mapping, QR URL parsing/generation, UserDefaults handling, memory zeroing utility, profile selection logic, dedicated password-message service behavior, ordinary-settings coordinator gating plus protected-settings schema v2 migration, self-test legacy cleanup, temporary/export/tutorial artifact cleanup, and ProtectedData framework coverage such as registry bootstrap/classification, wrapped-DMK contract checks, session relock behavior, startup seam validation, bootstrap outcome shaping, protected-data access-gate decisions, storage-root containment, explicit file-protection verification, fail-closed unsupported-volume handling, local-data reset, post-unlock key-metadata domain schema v2 creation/v1 migration/recovery, key custody capability resolver matrix and failure-category resolution coverage, Phase 3A/3B Secure Enclave custody handle-store creation/load/delete/inspect/inventory/reset-cleanup/recovery-classification behavior through mocks, Phase 4A/4B hidden generation, public-binding lookup, metadata/handle recovery report, and local-reset cleanup coverage, Phase 5A private-operation router routing/blocking/failure-mapping and signing-handle lookup coverage through mocks, Phase 5B cleartext signing routing coverage for unchanged software signing, hidden/test Secure Enclave signer routes, production policy blocks, no software fallback, auth/handle failure category mapping, and source-audit containment, Phase 5C text sign-plus-encrypt routing coverage for unsigned text not routing, unchanged software signed encryption, hidden/test Secure Enclave signer routes through a real catalog/router/public-binding inspector/shared mock handle store, production policy blocks, no software fallback, auth/handle failure category mapping, and unchanged file-streaming encryption behavior, Phase 5D password-message signing routing coverage for unsigned password encryption not routing, unchanged software signed password encryption, hidden/test Secure Enclave signer routes through a real catalog/router/public-binding inspector/shared mock handle store, production policy blocks, no software fallback, auth/handle failure category mapping, and unchanged password decrypt/tamper/no-SKESK/rejected-password behavior, Phase 5E detached file signing routing coverage for unchanged software detached signing, hidden/test Secure Enclave signer routes through a real catalog/router/public-binding inspector/shared mock handle store, production policy blocks, progress/auth/handle failure category mapping, no software fallback, and unchanged streaming file encryption behavior, Phase 5F streaming file encrypt-plus-sign routing coverage for unsigned file encryption not routing, unchanged software signed file encryption, hidden/test Secure Enclave signer routes through a real catalog/router/public-binding inspector/shared mock handle store, production policy blocks, explicit/default encrypt-to-self decrypt+verify coverage, progress/auth/handle/callback failure category mapping, output cleanup on failure, no software fallback, and unchanged text/password/detached/decrypt streaming behavior, Phase 5G modify-expiry routing coverage for unchanged software expiry mutation and recovery behavior, hidden/test Secure Enclave signer routes through a real catalog/router/public-binding inspector/shared mock handle store, production policy blocks, v4/v6 expiry set/remove coverage, no software unwrap or recovery journal path on Secure Enclave routes, auth/handle/callback failure category mapping, no software fallback, and explicit `refreshBinding` not-implemented behavior, Phase 5H selective revocation routing coverage for unchanged software subkey/User ID revocation export, public selector validation before routing, hidden/test Secure Enclave signer routes through a real catalog/router/public-binding inspector/shared mock handle store, production policy blocks, auth/handle/callback failure category mapping, no software fallback, no catalog/keychain mutation, and unchanged key-level stored revocation-artifact export behavior, Phase 5I contact certification routing coverage for unchanged software User ID certification, public selector validation before routing, hidden/test Secure Enclave signer routes through a real catalog/router/public-binding inspector/shared mock handle store, production policy blocks, auth/handle/callback failure category mapping, no software fallback, generated artifact validation, and no catalog/keychain/contact mutation, Phase 5J closure audit coverage for workflow-local custody switch containment, external P-256 signer runtime containment, expected 5B-5I helper operation routing, and explicit unsupported `refreshBinding` plus decrypt/key-agreement outcomes, protected-settings handoff-only auto-open behavior, private-key-control migration/recovery behavior, and the root-secret SE device-binding envelope through protocol-based mocks. Uses protocol-based mocks for Keychain and SE. v2 `CAPDSEV2` coverage belongs here first: seal/open round trip, field-length validation, HKDF sharedInfo mismatch, AAD mismatch, AAD-version rejection, ephemeral public-key binding, ciphertext/tag/nonce/salt/public-key tampering, v1-to-v2 migration, registry + Keychain `format-floor` downgrade rejection, `legacy-cleanup` deletion after the next successful v2 open, and Reset deleting the SE binding key.

Phase 6A adds macOS unit coverage for hidden/test decrypt routing, key-agreement handle lookup, Security bridge shared-secret derivation and failure mapping, FFI adapter error mapping, external P-256 key-agreement containment, and proof that `DecryptionService` did not yet consume the route. Phase 6B adds macOS unit coverage for the message decrypt consumer (`PrivateKeyMessageDecryptionService`): unchanged software-custody message decrypt without the key-agreement bridge, hidden/test Secure Enclave v4/v6 message round-trips through a real catalog/router/public-binding inspector/shared mock handle store without unwrapping a secret certificate, signed-message verification folding, production policy blocks, recipient mismatch and v4/v6 tamper fail-closed without partial plaintext, blocked/role-mismatch/callback failure category mapping, no software fallback, and the Phase 6 routing source-audit assertion. The existing `DecryptionServiceTests` software round-trip, tamper, and Phase 1/Phase 2 boundary coverage stays green through the router delegation. Phase 6D adds macOS unit closure coverage for the decrypt consumers: mixed-recipient and repeated message decrypt through `PrivateKeyMessageDecryptionService`, plus mixed-recipient, repeated, and no-partial-plaintext-on-tamper file decrypt through `PrivateKeyStreamingFileDecryptionService` — all without unwrapping a secret certificate or leaving a residual `.tmp` scratch artifact — and a `phase6WorkflowExternalKeyAgreementContainment` source-audit asserting `DecryptionService` never calls the external P-256 key-agreement runtime directly.
...
Phase 3A/3B custody handle-store tests stay in the macOS unit lane because those PRs are the Security boundary, local-reset cleanup, and mockable recovery-classification contract. Phase 3C adds guarded device evidence for real Secure Enclave handle creation/load/delete, biometric access-control private operations, unauthorized interaction failure, handle-state failures, and sanitized diagnostics. Phase 4A keeps hidden-generation service and public-binding lookup coverage in the macOS unit lane with mocks, Rust tests cover the public-only certificate/revocation construction matrix through the external signer boundary, and guarded device evidence validates the non-destructive hidden-generation path with a real signing handle. Phase 4B adds Rust public-binding inspection tests plus Swift recovery-report and reset-postcondition tests for metadata-only, handle-only, partial, ambiguous, wrong-public, corrupt-public-cert, and missing-revocation hidden states. Phase 4C covers public certificate export, stored key-level revocation export, missing-revocation fail-closed behavior, and private-key export/backup unsupported behavior for hidden Secure Enclave custody identities. Phase 5A stays in the macOS unit lane: it verifies the private-operation vocabulary, independent policy gates, software routes without handle lookup, hidden/test signer routes, blocked key-agreement/decrypt, sanitized failure mapping, Security signing-handle lookup, and source-audit containment for workflow-local custody switches. Phase 5B also stays in the macOS unit lane for the cleartext signing pilot, Phase 5C stays there for text sign-plus-encrypt routing, Phase 5D stays there for password-message optional signing, Phase 5E stays there for detached file signing, Phase 5F stays there for streaming file encrypt-plus-sign routing, Phase 5G stays there for modify-expiry binding signatures, Phase 5H stays there for selective subkey/User ID revocation signer routing, Phase 5I stays there for contact certification signer routing, and Phase 6A stays there for the key-agreement route foundation because it uses mocks and software P-256 keys to prove bridge semantics without releasing a plaintext workflow. Phase 6B stays in the macOS unit lane for in-memory message decrypt routing because it uses mocks and software P-256 keys to prove the consumer's software/Secure Enclave dispatch, v4/v6 round-trip, and fail-closed behavior without device hardware. Guarded device evidence is optional unless a later PR changes real-hardware behavior. The destructive real-hardware Reset All Local Data cleanup proof is isolated in `CypherAir-DangerousDeviceTests` because it deletes every app-owned Secure Enclave custody handle for the current app bundle, not only handles created by the test. Phase 6D adds `DeviceSecureEnclaveCustodyDecryptTests` to `CypherAir-DeviceTests`: it creates and deletes a single test-owned handle pair, then decrypts v4/v6 messages and a mixed-recipient file through a real `.keyAgreement` P-256 handle and confirms a tampered payload hard-fails with no output, reusing one authenticated `LAContext` for the generation and key-agreement private operations. Secure Enclave custody device tests require Secure Enclave plus enrolled biometrics — available on Apple Silicon / T2 Macs and SE-capable iPhones/iPads — and skip elsewhere; only the `DeviceMIETests` subset additionally requires A19/A19 Pro Hardware Memory Tagging.
...
- verify that Contacts creates an empty protected `contacts` domain after authorized unlock, treats corrupt or missing protected Contacts state as recovery, persists protected mutations across reopen, migrates schema v1 Contacts payloads by preserving contacts/keys/tags/certification artifacts while deleting `recipientLists` and writing back a verified schema v2 generation, and clears Contacts runtime state on relock or framework reset
