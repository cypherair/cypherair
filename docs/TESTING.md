...
- verify that ordinary settings stay locked before app authentication, load legacy values only after a healthy post-auth protected-settings handoff, enter recovery without reading legacy values when the domain/framework is unavailable, persist updates through the coordinator, clear snapshots on relock, and fail closed for resume grace while unavailable
...
- Later Phase 7 work must add coverage for ordinary-settings payload migration, legacy ordinary-setting key retirement, self-test state, and temporary/export/tutorial cleanup or file-protection changes.
