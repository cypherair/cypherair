...
Phase 3A custody handle-store tests stay in the macOS unit lane because the PR is only the Security boundary and mockable contract. Guarded device evidence for real Secure Enclave access-control prompts, handle-state failures, and hardware-specific cleanup belongs to the later PR 3C lane.
