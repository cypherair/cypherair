...
This summary is the entry point for the active Secure Enclave Custody planning
set. The completed POC documents are archived under
`docs/archive/apple-secure-enclave-custody-poc/` and remain historical evidence
roots when a decision needs the original validation context.
...
- Architecture Plan (`APPLE_SECURE_ENCLAVE_CUSTODY_ARCHITECTURE_PLAN.md`):
  high-level Swift/Rust/Security
...
The POC PR closeout record owns the short-term roadmap from this evidence track
to later production implementation PRs. If that roadmap becomes active
day-to-day guidance, it should be promoted into a dedicated active roadmap
document under the repository's documentation governance rules.
