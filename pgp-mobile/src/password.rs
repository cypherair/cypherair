...
pub fn decrypt(
    ...
    for skesk in &context.skesks {
        let Some((session_key_algo, session_key)) = derive_candidate(skesk, password)? else {
            continue;
        };
        ...
        match decrypt::decrypt_with_fixed_session_key(
            ...
            | Err(error @ PgpError::UnsupportedAlgorithm { .. }) => return Err(error),
...
fn collect_message_context(ciphertext: &[u8]) -> Result<PasswordMessageContext, PgpError> {
    ...
    let ppr = openpgp::parse::PacketParser::from_bytes(ciphertext).map_err(|e| PgpError::CorruptData {
    ...
    while let openpgp::parse::PacketParserResult::Some(pp) = ppr {
        match &pp.packet {
            openpgp::Packet::SKESK(skesk) => {
                validate_skesk(skesk)?;
                context.skesks.push(skesk.clone());
            }
...
fn derive_candidate(
    skesk: &SKESK,
    password: &Password,
) -> Result<Option<(Option<SymmetricAlgorithm>, SessionKey)>, PgpError> {
    match skesk.decrypt(password) {
        Ok((session_key_algo, session_key)) => {
            if let Some(algo) = session_key_algo {
                ...
                let expected_key_size =
                    algo.key_size()
                        .map_err(|_| PgpError::UnsupportedAlgorithm { algo: algo.to_string() })?;
                if session_key.len() != expected_key_size {
                    return Ok(None);
                }
            }

            Ok(Some((session_key_algo, session_key)))
        }
        Err(error) => classify_candidate_error(error),
    }
}

fn classify_candidate_error(
    error: openpgp::anyhow::Error,
) -> Result<Option<(Option<SymmetricAlgorithm>, SessionKey)>, PgpError> {
    if let Some(openpgp_error) = error.downcast_ref::<openpgp::Error>() {
        return match openpgp_error {
            openpgp::Error::UnsupportedSymmetricAlgorithm(algo) => Err(PgpError::UnsupportedAlgorithm {
                algo: algo.to_string(),
            }),
            openpgp::Error::UnsupportedAEADAlgorithm(algo) => Err(PgpError::UnsupportedAlgorithm {
                algo: algo.to_string(),
            }),
            ...
            | openpgp::Error::UnsupportedPacketType(_) => Err(PgpError::CorruptData {
