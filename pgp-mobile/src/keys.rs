...
///
/// There is no unspecified case. A caller wanting no expiry says `Never`, and no
/// Key Expiration Time subpacket is written; a caller wanting one states the
/// term. The engine holds no validity policy of its own on any path, so nothing
/// here can outlive or undercut what the caller asked for.
#[derive(Debug, Clone, Copy, PartialEq, Eq, uniffi::Enum)]
...
impl KeyValidity {
    /// The validity period Sequoia takes, whose `None` is precisely "never
    /// expires" (`CertBuilder::set_validity_period`,
...
pub use s2k::{export_s2k_params, parse_s2k_params, S2kInfo, S2kType};
