...
pub fn parse_key_info(key_data: &[u8]) -> Result<KeyInfo, PgpError> {
    ...
    // Whether this certificate can be encrypted to. The filter chain is the one
    // `encrypt::collect_recipients` applies, term for term: a difference here is
    // a contact the app offers as a recipient and then hard-fails on at send
    // time (or refuses to send to while the engine would have accepted).
