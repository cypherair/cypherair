...
/// their secrets returned for enveloping. Every binding signature is produced
/// through the external composite signer, which self-verifies before releasing a
/// signature — so a mismatched or broken Secure Enclave signing key fails
/// generation instead of producing an unusable certificate.
...
pub fn inspect_secure_enclave_composite_high_bindings(
    ...
    let primary = cert.primary_key().key();
    let (eddsa_signing_public_key, mldsa87_signing_public_key) = match primary.mpis() {
        mpi::PublicKey::MLDSA87_Ed448 { eddsa, mldsa } => (eddsa.to_vec(), mldsa.to_vec()),
        _ => {
            return Err(PgpError::InvalidKeyData {
                reason: "Split-custody composite · High primary key must be ML-DSA-87+Ed448"
                    .to_string(),
            })
        }
    };
    validate_mldsa87_public_key(&mldsa87_signing_public_key)?;

    let mut composite_key_agreement_subkeys = cert
        .keys()
        .subkeys()
        .filter_map(|subkey| match subkey.key().mpis() {
            mpi::PublicKey::MLKEM1024_X448 { ecdh, mlkem } => Some((
                subkey.key().fingerprint().to_hex().to_lowercase(),
                ecdh.to_vec(),
                mlkem.to_vec(),
            )),
    ...
    if composite_key_agreement_subkeys.is_empty() {
        return Err(PgpError::InvalidKeyData {
            reason:
                "Split-custody composite · High certificate is missing an ML-KEM-1024+X448 subkey"
                    .to_string(),
        });
    }
    if composite_key_agreement_subkeys.len() != 1 {
        return Err(PgpError::InvalidKeyData {
            reason:
                "Split-custody composite · High certificate must contain exactly one ML-KEM-1024+X448 subkey"
                    .to_string(),
        });
    }
    let (
    ...
    validate_mlkem1024_public_key(&mlkem1024_key_agreement_public_key)?;
    ...
    if !is_transport_encryption_capable {
        return Err(PgpError::InvalidKeyData {
            reason:
                "Split-custody composite · High key-agreement subkey is not transport-encryption capable"
                    .to_string(),
        });
    }
