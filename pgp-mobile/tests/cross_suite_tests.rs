...
fn test_legacy_encrypts_to_modern_high() {
    let sender_a = keys::generate_key_with_suite(
        "Alice (A)".to_string(),
        None,
        KeyValidity::Never,
...
fn test_cross_modern_high_to_a_signed_encrypted_round_trip() {
    ...
    // Per AGENTS.md hard constraint 8: v4 recipient → SEIPDv1 format auto-selected
    let ciphertext = encrypt::encrypt(
        plaintext,
        &[recipient_a.public_key_data.clone()],
        Some(&sender_b.cert_data),
...
/// This directly validates the AGENTS.md hard constraint 8 format auto-selection rule.
#[test]
