...
fn test_encrypt_decrypt_signed_profile_a() {
    ...
    assert_eq!(result.signer_fingerprint, Some(sender.fingerprint.clone()));
}
...
fn test_concurrent_encrypt_profile_a() {
    ...
    let handle1 = std::thread::spawn(move || {
        ...
        let result =
            decrypt::decrypt(&ct, &[k1_cert], &[]).expect("Concurrent decrypt 1 should succeed");
    ...
    let handle2 = std::thread::spawn(move || {
        ...
        let result =
            decrypt::decrypt(&ct, &[k2_cert], &[]).expect("Concurrent decrypt 2 should succeed");
...
fn test_modify_expiry_profile_a_extend() {
    ...
    assert_eq!(
        result.key_info.key_version, 4,
        "Profile A key version must remain 4"
    );
