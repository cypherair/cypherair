...
extension FFIIntegrationTests {
    ...
    func test_parseRecipients_legacy_returnsMatchingKeyIDs() throws {
        ...
        let key = try engine.generateKey(name: "Phase1 A", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
    ...
    func test_parseRecipients_modernHigh_returnsMatchingKeyIDs() throws {
        let engine = try XCTUnwrap(self.engine)
        let key = try engine.generateKey(name: "Phase1 B", email: nil, validity: .never, suite: .ed448X448)
        let ciphertext = try engine.encrypt(
    ...
    func test_twoPhaseDecrypt_noMatchingKey_phase1SucceedsPhase2Fails() throws {
        let engine = try XCTUnwrap(self.engine)
        let encryptKey = try engine.generateKey(name: "Encrypt Key", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
        let wrongKey = try engine.generateKey(name: "Wrong Key", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
    ...
    func test_twoPhaseDecrypt_multiRecipient_bothCanDecrypt() throws {
        let engine = try XCTUnwrap(self.engine)
        let alice = try engine.generateKey(name: "Alice Multi", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
        let bob = try engine.generateKey(name: "Bob Multi", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
    ...
    func test_matchRecipients_legacy_returnsPrimaryFingerprint() throws {
        let engine = try XCTUnwrap(self.engine)
        let key = try engine.generateKey(name: "Match A", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
    ...
    func test_matchRecipients_modernHigh_returnsPrimaryFingerprint() throws {
        let engine = try XCTUnwrap(self.engine)
        let key = try engine.generateKey(name: "Match B", email: nil, validity: .never, suite: .ed448X448)
    ...
    func test_matchRecipients_wrongCert_throwsNoMatchingKey() throws {
        let engine = try XCTUnwrap(self.engine)
        let encryptKey = try engine.generateKey(name: "Encrypt", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
        let wrongKey = try engine.generateKey(name: "Wrong", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
    ...
    func test_matchRecipients_multiRecipient_returnsAllMatches() throws {
        let engine = try XCTUnwrap(self.engine)
        let alice = try engine.generateKey(name: "Alice MR", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
        let bob = try engine.generateKey(name: "Bob MR", email: nil, validity: .never, suite: .ed25519LegacyCurve25519Legacy)
