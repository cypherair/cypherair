...
final class KeyMutationServiceSinglePromptExpiryTests: XCTestCase {
    ...
    /// Late-binds the service under test into its own expiry-authenticator stub.
    ...
    func test_modifyExpiry_catalogRelockedMidAction_throwsKeyMetadataUnavailable() async throws {
        ...
        // decrypt-flavored noMatchingKey, and must abort before any journal entry.
        let controlStore = InMemoryPrivateKeyControlStore(mode: .standard)
        let box = ServiceBox()
        let made = TestHelpers.makeKeyManagement(
            privateKeyControlStore: controlStore,
            expiryAuthenticator: { _, _ in
                box.service?.markKeyMetadataLocked()
                return LAContext()
            }
        )
        box.service = made.service
        let identity = try await made.service.generateKey(
            name: "Expiry Relock",
            email: nil,
            expirySeconds: nil,
        ...
        do {
            ...
            XCTFail("The modify flow must not reuse the decrypt-flavored noMatchingKey error")
        }

        XCTAssertNil(
            try controlStore.recoveryJournal().modifyExpiry,
            "The catalog guard aborts before any journal entry."
        )
    }
}
