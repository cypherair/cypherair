...
final class ContactServiceProtectedDomainTests: ContactServiceTestCase {
    ...
    func test_contactsFreshInstallCreatesEmptyProtectedDomain() async throws {
        ...
        XCTAssertTrue(
            FileManager.default.fileExists(
                atPath: harness.storageRoot.contactsSQLCipherDatabaseURL(for: ContactsDomainStore.domainID).path
            )
        )
        XCTAssertFalse(contactsDomainArtifactsExist(in: harness.storageRoot))
    ...
    func test_contactsCorruptProtectedDomainRequiresRecovery() async throws {
        ...
        try opened.harness.storageRoot.writeProtectedData(
            Data("not-a-readable-contacts-sqlcipher-database".utf8),
            to: opened.harness.storageRoot.contactsSQLCipherDatabaseURL(for: ContactsDomainStore.domainID)
        )
    ...
    func test_contactsMissingWrappedDomainMasterKeyRequiresRecoveryWithoutRecreation() async throws {
        let opened = try await makeOpenedProtectedContactService(prefix: "ContactsMissingWrappedDMK")
        defer {
            try? FileManager.default.removeItem(at: opened.harness.storageRoot.rootURL.deletingLastPathComponent())
        }
        let generated = try engine.generateKey(
        ...
        let reopenedStore = ContactsDomainStore(
            ...
            currentWrappingRootKey: { opened.harness.wrappingRootKey },
            initialSnapshotProvider: {
                XCTFail("Committed Contacts domain must not be recreated when the DMK row is missing.")
                return ContactsDomainSnapshot.empty()
            }
        )
        let reopenedService = ContactService(
    ...
    func test_contactsCorruptWrappedDomainMasterKeyRequiresRecoveryWithoutRecreation() async throws {
        let opened = try await makeOpenedProtectedContactService(prefix: "ContactsCorruptWrappedDMK")
        defer {
            try? FileManager.default.removeItem(at: opened.harness.storageRoot.rootURL.deletingLastPathComponent())
        }
        let generated = try engine.generateKey(
        ...
        try await opened.service.relockProtectedData()
        opened.harness.domainKeyManager.clearUnlockedDomainMasterKeys()
        try opened.harness.keychain.update(
            Data("not a wrapped contacts dmk".utf8),
            service: KeychainConstants.protectedDataDomainKeyService(domainID: ContactsDomainStore.domainID),
        ...
        let reopenedStore = ContactsDomainStore(
            ...
            currentWrappingRootKey: { opened.harness.wrappingRootKey },
            initialSnapshotProvider: {
                XCTFail("Committed Contacts domain must not be recreated when the DMK row is corrupt.")
                return ContactsDomainSnapshot.empty()
            }
        )
        let reopenedService = ContactService(
    ...
    func test_contactsCorruptSQLCipherDatabaseAfterMutationRequiresRecovery() async throws {
        ...
        let contactsDirectory = documentDirectory.appendingPathComponent("contacts", isDirectory: true)
        let harness = try makeContactsProtectedHarness(
            prefix: "ContactsPR4CorruptSQLCipher",
            contactsDirectory: contactsDirectory
        )
        defer {
            try? FileManager.default.removeItem(at: harness.storageRoot.rootURL.deletingLastPathComponent())
        ...
        try harness.storageRoot.writeProtectedData(
            Data("not-a-readable-contacts-sqlcipher-database".utf8),
            to: harness.storageRoot.contactsSQLCipherDatabaseURL(for: ContactsDomainStore.domainID)
        )
    ...
    func test_contactsMissingSQLCipherDatabaseAfterMutationRequiresRecovery() async throws {
        ...
        let contactsDirectory = documentDirectory.appendingPathComponent("contacts", isDirectory: true)
        let harness = try makeContactsProtectedHarness(
            prefix: "ContactsPR4MissingSQLCipher",
            contactsDirectory: contactsDirectory
        )
        defer {
            try? FileManager.default.removeItem(at: harness.storageRoot.rootURL.deletingLastPathComponent())
