...
/// than re-derived from the inputs the prediction under test also used.
///
...
final class EncryptScreenModelTests: XCTestCase {
    ...
    /// prediction below is checked against the SEIPD version of the ciphertext
    /// the same model then produces. Each configuration is one the indicator got
    /// wrong while it read the sender's default key version instead of the
    /// recipient set.
    @MainActor
    func test_outgoingMessageFormatPreview_matchesTheSeipdVersionTheEncryptPathProduces() async throws {
        ...
        // All-v6 recipients: AEAD, and nobody is holding the format back.
        model.selectedRecipients = [modernId]
        var preview = model.outgoingMessageFormatPreview
        XCTAssertEqual(preview.format, .seipdV2)
        XCTAssertFalse(preview.downgradesAeadCapableRecipients)
        XCTAssertFalse(preview.limitsFormat(recipientKeyVersion: 6))
        try await assertEncryptedSeipdVersion(2, model: model, description: "v6-only selection")

        // Mixed: the v4 recipient pulls the whole message down to SEIPDv1 and is
        // the row worth marking. The default key is Legacy at this point, so the
        // old sender-default predicate called every row compatible.
        model.selectedRecipients = [modernId, legacyId]
        preview = model.outgoingMessageFormatPreview
        XCTAssertEqual(preview.format, .seipdV1)
        XCTAssertTrue(preview.downgradesAeadCapableRecipients)
        XCTAssertTrue(preview.limitsFormat(recipientKeyVersion: 4))
        XCTAssertFalse(preview.limitsFormat(recipientKeyVersion: 6))
        try await assertEncryptedSeipdVersion(1, model: model, description: "mixed selection")

        // All-v4 recipients while the default key is v6: SEIPDv1, but nothing was
        // given up, so nothing is flagged. The old predicate warned here.
        try stack.keyManagement.setDefaultKey(fingerprint: modernSelf.fingerprint)
        model.selectedRecipients = [legacyId]
        preview = model.outgoingMessageFormatPreview
        XCTAssertEqual(preview.format, .seipdV1)
        XCTAssertFalse(preview.downgradesAeadCapableRecipients)
        XCTAssertFalse(preview.limitsFormat(recipientKeyVersion: 4))
        try await assertEncryptedSeipdVersion(1, model: model, description: "v4-only selection")

        // The Encrypt to Self copy is a recipient too: a v4 self key downgrades a
        // v6-only selection, it is the explicitly chosen key rather than the (v6)
        // default, and the key doing it has no row to carry a warning.
        model.selectedRecipients = [modernId]
        model.encryptToSelf = true
        ...
        XCTAssertTrue(preview.downgradesAeadCapableRecipients)
        XCTAssertFalse(preview.limitsFormat(recipientKeyVersion: 6))
        try await assertEncryptedSeipdVersion(1, model: model, description: "v4 Encrypt to Self copy")
    }
    ...
    private func assertEncryptedSeipdVersion(
        ...
        XCTAssertEqual(
            version,
            expectedVersion,
            "\(description) was sent as SEIPDv\(version), not the previewed SEIPDv\(expectedVersion)",
            file: file,
            line: line
        )
    }
