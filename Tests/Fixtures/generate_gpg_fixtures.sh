#!/usr/bin/env bash
# Generate GnuPG interoperability test fixtures for CypherAir.
# These fixtures validate GnuPG interop and DEFLATE decryption.
#
...
# ── 1. Generate a GnuPG Ed25519 key (compatible with Legacy) ──
...
# Export GnuPG public key (App imports gpg pubkey)
...
# ── 2. GnuPG encrypts a message (App decrypts gpg-encrypted message) ──
...
# Sequoia Legacy key, export it, and we provide gpg-encrypted-to-gpg-key fixtures
...
# ── 3. GnuPG signs a message (App verifies gpg signature) ──
...
# ── 5. GnuPG encrypts with DEFLATE compression ──
...
# ── 7. Tampered ciphertext (tamper 1 bit → gpg fails) ──
...
# ── 8. Test GnuPG rejection of Modern High v6 key ──
...
if [ -f "$SCRIPT_DIR/modern_high_v6_pubkey.gpg" ]; then
    echo "--- Testing GnuPG rejection of v6 key ---"
    # GnuPG 2.4.x cannot import v6 keys. Capture the exact error output.
    if gpg --import "$SCRIPT_DIR/modern_high_v6_pubkey.gpg" > "$SCRIPT_DIR/gpg_v6_import_rejection.txt" 2>&1; then
