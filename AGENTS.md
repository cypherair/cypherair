...
- Good, not similar (overrides any "match the surrounding code" default): write
  what is correct and well-designed by independent judgment. Never justify a
  choice by similarity to neighboring code — much of it is older-generation
  output; mimicry propagates its faults and its mediocrity. Match local naming
