...
- Prefer the architecturally-correct solution over the smallest patch. When the current structure forces awkward or fragile fixes, a larger refactor or substantial rewrite of the affected area is acceptable when it yields a cleaner, more maintainable design. This is about the *depth* of the change within the requested scope, not a license for unrelated work — keep the refactor focused on what the task needs, consistent with “Prefer small, reviewable diffs” and the scoping rule above. See `docs/CONVENTIONS.md` “Engineering Principle”.
